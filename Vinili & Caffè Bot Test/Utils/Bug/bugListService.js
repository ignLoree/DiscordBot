/**
 * Lista bug solo nella guild test. Canale fisso; ordine per gravità (online > inattivo > pausa > offline).
 */
const fs = require('fs');
...
function save(data) {
  ...
  fs.writeFileSync(BUG_FILE, JSON.stringify({ items: data.items, messageId: data.messageId }, null, 2), 'utf8');
}
