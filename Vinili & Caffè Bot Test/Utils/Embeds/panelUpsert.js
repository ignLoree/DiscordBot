...
function toComparableEmbeds(embeds = []) {
  return JSON.stringify(
    (Array.isArray(embeds) ? embeds : []).map(simplifyEmbed).filter(Boolean),
  );
}
...
async function shouldEditMessage(message, payload) {
  ...
  if (
    toComparableEmbeds(message?.embeds || []) !==
    toComparableEmbeds(payload?.embeds || [])
  )
    return true;
  if (
    toComparableComponents(message?.components || []) !==
    toComparableComponents(payload?.components || [])
  )
    return true;
  return false;
}
