...
function toComparableEmbeds(embeds = []) {
  return JSON.stringify((Array.isArray(embeds) ? embeds : []).map(simplifyEmbed).filter(Boolean));
}
...
async function upsertPanelMessage(channel, client, payload) {
  ...
  if (messageId) {
    ...
    if (direct) {
      ...
      if (await shouldEditMessage(direct, editPayload)) await direct.edit(editPayload).catch(() => {});
      return direct;
    }
  }
  ...
  let existing = payloadCustomIds.size > 0 ? botMessages.find((msg) => {
  ...
  if (!existing) return await channel.send(cleanPayload).catch(() => null);
  if (await shouldEditMessage(existing, cleanPayload)) await existing.edit(cleanPayload).catch(() => {});
  return existing;
}
