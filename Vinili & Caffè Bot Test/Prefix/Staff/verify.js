...
function buildPromptEmbed(targetTag) {
  ...
  return new EmbedBuilder()
    ...
    .setDescription(
      `<:rightDoubleArrow:1465450678062288937> Target:\n${targetText}`,
    );
}
...
async function runVerifyCommand(message, args) {
    ...
    await safeMessageReply(message, {
      embeds: [
        new EmbedBuilder()
          .setColor(ERROR_COLOR)
          .setDescription(
            "<:vegax:1472992044140990526> Solo lo **staff** può usare il comando `-verify` su questo server.",
          ),
      ],
  ...
  if (!targets.length) {
    await message.reply({ embeds: [buildNoMemberEmbed()] }).catch(() => null);
    await message.delete().catch(() => {});
    return true;
  }

  const role =
    message.guild.roles.cache.get(verifyRoleId) ||
    (await message.guild.roles.fetch(verifyRoleId).catch(() => null));
      ...
      .send({
        embeds: [
          new EmbedBuilder()
            ...
            .setTitle("Unsuccessful Operation!")
            .setDescription(
              "Ruolo verifica non configurato per questo server (verificatoRoleIds).",
            ),
        ],
      })
      .catch(() => null);
  ...
  collector.on("collect", async (i) => {
    ...
    setTimeout(() => resultMsg.delete().catch(() => {}), RESULT_DELETE_MS);
  });
