...
async function resolveVoiceChannel(guild, doc) {
  ...
  if (doc.customVocChannelId) {
    channel = guild.channels.cache.get(doc.customVocChannelId)
...
async function canManageChannels(guild) {
  ...
  const me = guild.members.me || await guild.members.fetchMe().catch(() => null);
  return Boolean(me?.permissions?.has(PermissionsBitField.Flags.ManageChannels));
}
...
async function processExpiredCustomRole(client, doc) {
  ...
  let role = guild.roles.cache.get(doc.roleId) || await guild.roles.fetch(doc.roleId).catch(() => null);
  ...
  if (role) {
    if (await canManageRole(guild, role)) {
      await role.delete(`Custom role expired for user ${doc.userId}`).catch(() => {});
      role = guild.roles.cache.get(doc.roleId) || await guild.roles.fetch(doc.roleId).catch(() => null);
      roleHandled = !role;
    }
  }
  ...
  if (voiceChannel) {
    if (await canManageChannels(guild)) {
      await voiceChannel.delete(`Custom private voice expired for user ${doc.userId}`).catch(() => {});
      const stillExists = guild.channels.cache.get(voiceChannel.id)
  ...
  if (channelHandled && doc.customVocChannelId) {
    await CustomRole.updateOne({ _id: doc._id }, { $set: { customVocChannelId: null } }).catch(() => {});
  }
