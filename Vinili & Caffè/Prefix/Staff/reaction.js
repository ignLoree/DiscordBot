...
async function handleAutoResponders(message, args) {
  ...
  if (sub === 'remove' || sub === 'del' || sub === 'rm') {
    ...
    await safeMessageReply(message, {
      embeds: [new EmbedBuilder().setColor('#6f4e37').setDescription(`<:vegacheckmark:1443666279058772028> Trigger rimossi: ${removedLabel}`)],
      allowedMentions: { repliedUser: false }
    });
    return;
  }
  ...
  if (sub === 'add' || sub === 'set' || sub === 'edit') {
    ...
    if (!triggerList.length) {
      await safeMessageReply(message, {
        content: '<:vegax:1443934876440068179> Trigger mancante. Usa: `+reaction auto add trigger | risposta | reaction`',
        allowedMentions: { repliedUser: false }
      });
      return;
    }
    ...
    const existingDocs = await AutoResponder.find({
      guildId,
      triggerLower: { $in: triggerLowerList }
    }).select('triggerLower').lean().catch(() => []);
  ...
  await safeMessageReply(message, {
    content: '<:vegax:1443934876440068179> Sottocomando non valido. Usa `+reaction auto help`.',
    allowedMentions: { repliedUser: false }
  });
}
