...
function buildRows(userId) {
  const row1 = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId(`eb:title:${userId}`).setLabel('Titolo').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId(`eb:description:${userId}`).setLabel('Descrizione').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId(`eb:color:${userId}`).setLabel('Colore').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId(`eb:footer:${userId}`).setLabel('Footer').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId(`eb:author:${userId}`).setLabel('Autore').setStyle(ButtonStyle.Secondary)
  );
...
module.exports = {
  ...
  async execute(message, args = []) {
    ...
    if (!preview) {
      return safeMessageReply(message, {
        content: '<:vegax:1443934876440068179> Non riesco a creare il builder qui.',
        allowedMentions: { repliedUser: false }
      });
    }

    if (!message.client.embedBuilderSessions) message.client.embedBuilderSessions = new Map();
