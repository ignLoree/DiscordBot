...
async function trySetSponsorNickname(member, guildId) {
  try {
    if (!member) return;
    if (!guildId) return;
    if (!isSponsorGuild(guildId)) return;

    if ((member.nickname || '') === SPONSOR_VERIFY_NICKNAME) return;
    if (member.manageable === false) return;

    await member.setNickname(SPONSOR_VERIFY_NICKNAME).catch((err) => {
      global.logger?.warn?.('[VERIFY] Failed to set sponsor nickname:', err);
    });
  } catch (err) {
    global.logger?.warn?.('[VERIFY] Sponsor nickname handler error:', err);
  }
}
...
async function resolveValidVerifyRoleIds(guild) {
  ...
  } else if (isSponsorGuild(gid)) {
    const sponsorRole = VERIFICATO_ROLE_IDS[gid];
    roleIds = sponsorRole ? [sponsorRole] : [];
  } else {
    roleIds = [];
  }
...
function isAlreadyVerifiedInThisGuild(member, guildId) {
  ...
  if (isSponsorGuild(guildId)) {
    const rid = VERIFICATO_ROLE_IDS[guildId];
...
async function finalizeVerification(interaction, member) {
  ...
  if (isSponsorGuild(guildId)) {
    await trySetSponsorNickname(member, guildId);
  }
