...
function buildRows(ownerId) {
  ...
  const row1 = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId(`eb:title:${ownerId}`).setLabel('Titolo').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId(`eb:description:${ownerId}`).setLabel('Descrizione').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId(`eb:color:${ownerId}`).setLabel('Colore').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId(`eb:footer:${ownerId}`).setLabel('Footer').setStyle(ButtonStyle.Secondary),
...
function buildEditModal(kind, ownerId, messageId, currentValue) {
  ...
  if (kind === 'description') {
    ...
    input.setLabel('Colore HEX es: #6f4e37 (vuoto = default)').setStyle(TextInputStyle.Short).setRequired(false);
  } else if (kind === 'thumbnail') {
    input.setLabel('Thumbnail URL (vuoto = rimuovi)').setStyle(TextInputStyle.Short).setRequired(false);
  } else if (kind === 'image') {
    input.setLabel('Image URL (vuoto = rimuovi)').setStyle(TextInputStyle.Short).setRequired(false);
  } else if (kind === 'footer') {
    input.setLabel('Footer (vuoto = rimuovi)').setStyle(TextInputStyle.Short).setRequired(false);
  } else if (kind === 'author') {
    input.setLabel('Autore (vuoto = rimuovi)').setStyle(TextInputStyle.Short).setRequired(false);
  } else {
    input.setLabel('Titolo (vuoto = rimuovi)').setStyle(TextInputStyle.Short).setRequired(false);
  }
...
function buildSendModal(ownerId, messageId) {
  const modal = new ModalBuilder().setCustomId(`ebm:send:${ownerId}:${messageId}`).setTitle('Invia embed');
...
async function handleEmbedBuilderInteraction(interaction, client) {
  ...
  if (interaction.isButton && interaction.isButton()) {
    ...
    if (interaction.user.id !== ownerId) {
      await interaction.reply({ content: '<:vegax:1443934876440068179> Questo builder non è tuo.', flags: 1 << 6 }).catch(() => {});
      return true;
    }

    const session = getSession(client, messageId);
    if (!session) {
      await interaction.reply({ content: '<:vegax:1443934876440068179> Sessione scaduta o non trovata.', flags: 1 << 6 }).catch(() => {});
      return true;
    }

    if (kind === 'send') {
      await interaction.showModal(buildSendModal(ownerId, messageId)).catch(() => {});
      return true;
    }
  ...
  if (interaction.isModalSubmit && interaction.isModalSubmit()) {
    ...
    if (kind === 'send') {
      ...
      if (!channel?.isTextBased?.()) {
        await interaction.reply({ content: '<:vegax:1443934876440068179> Canale non valido.', flags: 1 << 6 }).catch(() => {});
        return true;
      }
      ...
      if (me) {
        ...
        if (!perms?.has(PermissionsBitField.Flags.SendMessages)) {
          await interaction.reply({ content: '<:vegax:1443934876440068179> Non ho permesso di scrivere in quel canale.', flags: 1 << 6 }).catch(() => {});
          return true;
        }
      }
      ...
      await interaction.reply({ content: '✅ Embed inviato!', flags: 1 << 6 }).catch(() => {});
      return true;
    }
    ...
    if (!value) {
      ...
      if (!normalized) {
        await interaction.reply({ content: '<:vegax:1443934876440068179> Colore non valido. Usa tipo `#6f4e37`.', flags: 1 << 6 }).catch(() => {});
        return true;
      }
      session.embed.color = normalized;
    } else if ((kind === 'thumbnail' || kind === 'image') && !isValidUrl(value)) {
