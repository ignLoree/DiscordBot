...
function buildGlobalDisplayName(entry, client) {
  if (!entry.privacyGlobal) return "Private user";
  const member = client.users.cache.get(entry.discordId);
  return member?.username || entry.lastFmUsername || "Private user";
}
  ...
  async execute(message, args) {
    ...
    try {
      ...
      const fullResults = checks
        .filter(Boolean)
        .map(entry => {
          const displayName = buildGlobalDisplayName(entry, message.client);
      ...
      const title = `${resolved.track} by ${resolved.artist} globally`;
      ...
      const notice = requester.privacyGlobal === false
        ...
        : null;
      const footerLine = `Global track - ${totalListeners} listeners - ${totalPlays} plays - ${avgPlays} avg`;
