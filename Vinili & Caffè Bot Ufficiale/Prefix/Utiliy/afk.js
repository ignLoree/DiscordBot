...
module.exports = {
    ...
    async execute(message, args) {
        ...
        try {
            ...
            if (!member) return safeMessageReply(message, "<:vegax:1443934876440068179> Errore: non posso trovare il tuo profilo.");
            const originalName = member.nickname || message.author.username;
            await AFK.findOneAndUpdate(
                { guildId, userId },
                { guildId, userId, message: afkMessage, timestamp: Date.now(), originalName },
                { upsert: true }
            );
            if (!originalName.startsWith("[AFK]")) {
                await member.setNickname(`[AFK] ${originalName}`).catch(() => { });
            }
            safeMessageReply(message, `<@${userId}> Ho impostato il tuo stato AFK: __**${afkMessage}**__`);
        } catch (error) {
