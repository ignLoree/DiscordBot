...
module.exports = {
  ...
  async execute(message, args = [], client) {
      ...
      try {
        ...
        const safeOpenerName =
          String(openerName)
            .replace(/[^\w.-]/g, "")
            .slice(0, 20) || "utente";
        const newChannelName = `ticket-${panelConfig.name}-${safeOpenerName}`.slice(0, 100);
    ...
    if (subcommand === "rename") {
      ...
      const currentParts = currentName.split("-").filter(Boolean);
      const ticketPrefixBase = currentParts.length >= 2
        ? currentParts.slice(0, 2).join("-")
        : "ticket-custom";
      if (!ticketPrefixBase) {
        await safeMessageReply(message, {
          embeds: [
            makeErrorEmbed(
              "Errore",
              "<:vegax:1443934876440068179> Nome canale ticket non valido.",
            ),
          ],
          allowedMentions: { repliedUser: false },
        });
      ...
      const ticketPrefix = `${ticketPrefixBase}-`;
