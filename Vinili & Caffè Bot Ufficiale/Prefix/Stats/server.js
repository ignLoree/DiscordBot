...
async function buildServerOverviewPayload(
  ...
  let file = null;
  try {
    const buffer=await renderServerActivityCanvas({guildName:guild?.name||"Server",guildIconUrl:guild?.iconURL({extension:"png",size:256})||null,createdOn:guild?.createdAt||null,invitedBotOn:guild?.members?.me?.joinedAt||null,lookbackDays,windows:{d1,d7,d14:dLookback,[lookbackKey]:dLookback,},topUsersText:enriched.topUsersText,topUsersVoice:enriched.topUsersVoice,topChannelsText:enriched.topChannelsText,topChannelsVoice:enriched.topChannelsVoice,chart:Array.isArray(stats?.chart)?stats.chart:[],approximate:Boolean(stats?.approximate),});
  ...
  const embed = new EmbedBuilder().setColor("#6f4e37").setAuthor({ name: `${guild?.name || "Server"} - Overview ${lookbackDays}d`, iconURL: guild?.iconURL({ size: 128 }) || null }).setImage(file ? `attachment://${imageName}` : null).setDescription([`Messaggi (1d/7d/${lookbackDays}d): **${d1.text} / ${d7.text} / ${dLookback.text}**`, `Ore vocali (1d/7d/${lookbackDays}d): **${formatHours(d1.voiceSeconds)} / ${formatHours(d7.voiceSeconds)} / ${formatHours(dLookback.voiceSeconds)}**`, `Contributori (1d/7d/${lookbackDays}d): **${d1.contributors} / ${d7.contributors} / ${dLookback.contributors}**`, stats?.approximate ? "_Nota: dati retroattivi parziali._" : null].filter(Boolean).join("\n"));
...
module.exports = {
  ...
  async execute(message, args = []) {
    ...
    const payload=await buildServerOverviewPayload(message.guild,lookbackDays,wantsEmbed,message.author?.id,);
