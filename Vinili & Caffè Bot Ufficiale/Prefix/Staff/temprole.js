...
async function resolveRole(message, raw) {
  ...
  const id = String(raw || '').replace(/[<@&>]/g, '');
  if (!/^\d{16,20}$/.test(id)) return null;
  return message.guild.roles.cache.get(id) || message.guild.roles.fetch(id).catch(() => null);
}
...
module.exports = {
  name: 'temprole',
  aliases: ['trole', 'temprolegrant', 'temprolerevoke', 'temprolelist', 'temproleclear'],
  ...
  async execute(message, args = []) {
    ...
    if (!target || target.bot) {
      return safeMessageReply(message, {
        embeds: [
          new EmbedBuilder()
            .setColor('Red')
            .setDescription('<:vegax:1443934876440068179> Utente non valido. Usa mention o ID.')
        ],
        allowedMentions: { repliedUser: false }
      });
    }
      ...
      if (!role) {
        return safeMessageReply(message, {
          embeds: [
            new EmbedBuilder()
              .setColor('Red')
              .setDescription('<:vegax:1443934876440068179> Ruolo non valido. Usa mention o ID.')
          ],
          allowedMentions: { repliedUser: false }
        });
      }
      ...
      if (!durationMs) {
        return safeMessageReply(message, {
          embeds: [
            new EmbedBuilder()
              .setColor('Red')
              .setDescription('<:vegax:1443934876440068179> Durata non valida. Esempi: `30m`, `2h`, `3d`.')
          ],
          allowedMentions: { repliedUser: false }
        });
      }
      ...
      if (!result.ok) {
        return safeMessageReply(message, {
          embeds: [
            new EmbedBuilder()
              .setColor('Red')
              .setDescription(`<:vegax:1443934876440068179> ${mapGrantError(result.reason)}`)
          ],
          allowedMentions: { repliedUser: false }
        });
      }
      ...
      if (!result.ok) {
        return safeMessageReply(message, {
          embeds: [
            new EmbedBuilder()
              .setColor('Red')
              .setDescription('<:vegax:1443934876440068179> Revoca non riuscita.')
          ],
          allowedMentions: { repliedUser: false }
        });
      }
      ...
      return safeMessageReply(message, {
        embeds: [
          new EmbedBuilder()
            ...
            .setDescription(`Record rimossi per ${target}: **${Number(result?.removed || 0)}**`)
        ],
        allowedMentions: { repliedUser: false }
      });
    }
    ...
    const lines = rows.length
      ? rows.map((row) => `- <@&${row.roleId}> -> scade tra **${formatRemaining(row.expiresAt)}**`)
