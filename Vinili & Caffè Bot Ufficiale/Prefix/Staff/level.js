...
module.exports = {
  ...
  async execute(message, args = []) {
    ...
    if (sub === 'config') {
      ...
      const lockList = settings.lockedChannelIds.length ? settings.lockedChannelIds.map((id) => `<#${id}>`).join(', ') : 'Nessuno';
      const ignoreList = settings.ignoredRoleIds.length ? settings.ignoredRoleIds.map((id) => `<@&${id}>`).join(', ') : 'Nessuno';
    ...
    if (sub === 'lock' || sub === 'unlock') {
      ...
      if (!/^\d{16,20}$/.test(channelId)) {
        await safeMessageReply(message, { content: '<:vegax:1443934876440068179> Usa: `+level lock <#canale|id>`', allowedMentions: { repliedUser: false } });
        return;
      }
      ...
      const embed = new EmbedBuilder()
        .setColor('#6f4e37')
        .setDescription(`${locked ? 'Canale bloccato' : 'Canale sbloccato'} per EXP: <#${channelId}>. Totale canali lock: **${next.length}**`);
      await safeMessageReply(message, { embeds: [embed], allowedMentions: { repliedUser: false } });
      return;
    }
    ...
    if (sub === 'ignore' || sub === 'unignore') {
      const role = message.mentions?.roles?.first()
        || message.guild.roles.cache.get(String(args[1] || '').replace(/[<@&>]/g, ''))
        || null;
      if (!role) {
        await safeMessageReply(message, { content: '<:vegax:1443934876440068179> Usa: `+level ignore <@ruolo|id>`', allowedMentions: { repliedUser: false } });
        return;
      }
      ...
      const embed = new EmbedBuilder()
        .setColor('#6f4e37')
        .setDescription(`${ignored ? 'Ruolo ignorato' : 'Ruolo riabilitato'} per EXP: ${role}. Totale ruoli ignorati: **${next.length}**`);
      await safeMessageReply(message, { embeds: [embed], allowedMentions: { repliedUser: false } });
      return;
    }
    ...
    if (sub === 'multiplier') {
      ...
      if (!Number.isFinite(value) || value <= 0 || !Number.isFinite(minutes) || minutes <= 0) {
        await safeMessageReply(message, { content: '<:vegax:1443934876440068179> Usa: `+level multiplier <valore> [minuti]`', allowedMentions: { repliedUser: false } });
        return;
      }
      const result = await setTemporaryEventMultiplier(guildId, value, minutes * 60 * 1000);
      const embed = new EmbedBuilder()
        .setColor('#6f4e37')
        .setDescription(`Evento EXP impostato a **${result.multiplier}x** fino al **${fmtDate(result.expiresAt)}**.`);
      await safeMessageReply(message, { embeds: [embed], allowedMentions: { repliedUser: false } });
      return;
    }
    ...
    if (sub === 'set') {
      ...
      if (!['exp', 'level'].includes(mode) || !Number.isFinite(value) || value < 0) {
        await safeMessageReply(message, { content: '<:vegax:1443934876440068179> Usa: `+level set <@utente|id> <exp|level> <valore>`', allowedMentions: { repliedUser: false } });
        return;
      }
      ...
      if (!Number.isFinite(delta) || delta <= 0) {
        await safeMessageReply(message, { content: '<:vegax:1443934876440068179> Usa: `+level add <@utente|id> <exp>`', allowedMentions: { repliedUser: false } });
        return;
      }
      ...
      const delta = asInt(args[2]);
      if (!Number.isFinite(delta) || delta <= 0) {
        await safeMessageReply(message, { content: '<:vegax:1443934876440068179> Usa: `+level remove <@utente|id> <exp>`', allowedMentions: { repliedUser: false } });
        return;
      }
      ...
      await safeMessageReply(message, { content: '<:vegax:1443934876440068179> Subcomando non valido.', allowedMentions: { repliedUser: false } });
      return;
    }
