...
module.exports = {
  ...
  async execute(message, args = []) {
    ...
    let totalExp = 0;
    try {
      const doc = await MinigameUser.findOne({
    ...
    const unlockedText = unlocked.length
      ? unlocked
          .map(
            (reward) => `<a:VC_Arrow:1448672967721615452> <@&${reward.roleId}>`,
          )
          .join("\n")
    ...
    const embed = new EmbedBuilder()
      ...
      .setTitle(
        `Le statistiche nei Minigames di ${targetUser.username} <a:VC_Flowers:1468687836055212174>`,
      )
      .setDescription(
        [
