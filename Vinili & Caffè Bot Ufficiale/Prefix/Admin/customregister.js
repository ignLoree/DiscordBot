...
function parseUser(message, raw) {
  ...
  if (!raw) return null;
  const id =
    String(raw).match(/^<@!?(\d+)>$/)?.[1] ||
    (String(raw).match(/^\d{17,20}$/) ? String(raw) : null);
  if (!id) return null;
...
module.exports = {
  ...
  async execute(message, args = []) {
        ...
        await safeMessageReply(message, {
          embeds: [
            new EmbedBuilder()
              .setColor("Red")
              .setDescription(
                "<:vegax:1443934876440068179> Uso: `+customregister role @utente @ruolo`.",
              ),
          ],
          allowedMentions: { repliedUser: false },
        });
      ...
      await safeMessageReply(message, {
        embeds: [
          new EmbedBuilder()
            ...
            .setDescription(
              `Associato ${role} a ${user} nel database custom role.`,
            ),
        ],
        allowedMentions: { repliedUser: false },
      });
      ...
      await safeMessageReply(message, {
        embeds: [
          new EmbedBuilder()
            .setColor("Red")
            .setDescription(
              "<:vegax:1443934876440068179> Uso: `+customregister voc #canale @utente @ruolo`.",
            ),
        ],
        allowedMentions: { repliedUser: false },
      });
    ...
    if (!effectiveRole) {
      ...
      if (doc?.roleId) {
        effectiveRole =
          message.guild.roles.cache.get(doc.roleId) ||
          (await message.guild.roles.fetch(doc.roleId).catch(() => null));
      }
    }
    if (!effectiveRole) {
      await safeMessageReply(message, {
        embeds: [
          new EmbedBuilder()
            .setColor("Red")
            .setDescription(
              "<:vegax:1443934876440068179> Ruolo non trovato. Passa anche `@ruolo` nel comando.",
            ),
        ],
        allowedMentions: { repliedUser: false },
      });
    ...
    await CustomRole.findOneAndUpdate(
      { guildId: message.guild.id, userId: user.id },
      {
        $set: {
          ...
          customVocChannelId: voiceChannel.id,
        },
      },
      { upsert: true, new: true, setDefaultsOnInsert: true },
    ).catch(() => null);
    ...
    await voiceChannel.permissionOverwrites
      .edit(message.guild.roles.everyone.id, {
        ViewChannel: false,
        Connect: false,
        Speak: false,
      })
    ...
    const inExpectedCategory =
      voiceChannel.parentId === CUSTOM_VOICE_CATEGORY_ID;
