...
module.exports = {
  ...
  async execute(message, args) {
    ...
    try {
      ...
      if (!data || data.Response === "False") return replyError(message, "Film non trovato.");
      return safeMessageReply(message, {
        embeds: [{
          color: 0x8e44ad,
          title: String((data.Title || query) + " (" + (data.Year || "N/D") + ")"),
          description: clamp(data.Plot || "Trama non disponibile."),
          thumbnail: data.Poster && data.Poster !== "N/A" ? { url: data.Poster } : undefined,
          fields: [
            { name: "Rating", value: String(data.imdbRating || "N/D"), inline: true },
