  ...
  async execute(message, args) {
    ...
    try {
      ...
      return safeMessageReply(message, {
        embeds: [{
          color: 0xe74c3c,
          title: String((track.trackName || "Unknown") + " - " + (track.artistName || "Unknown")),
          url: track.trackViewUrl || track.collectionViewUrl || undefined,
          description: clamp(track.collectionName || "N/D"),
          thumbnail: track.artworkUrl100 ? { url: track.artworkUrl100.replace("100x100", "600x600") } : undefined,
          fields: [
            { name: "Album", value: String(track.collectionName || "N/D"), inline: true },
            { name: "Genere", value: String(track.primaryGenreName || "N/D"), inline: true },
            { name: "Durata", value: String(Math.round(Number(track.trackTimeMillis || 0) / 1000) + "s"), inline: true },
          ],
        }],
