...
module.exports = {
  ...
  async execute(message, args) {
    ...
    try {
      ...
      const types = Array.isArray(data?.types) ? data.types.map((t) => t?.type?.name).filter(Boolean).join(", ") : "N/D";
      const abilities = Array.isArray(data?.abilities) ? data.abilities.map((a) => a?.ability?.name).filter(Boolean).slice(0, 4).join(", ") : "N/D";
      return safeMessageReply(message, {
        embeds: [{
          color: 0xf1c40f,
          title: "Pokemon: " + String(data?.name || query).toUpperCase(),
          thumbnail: data?.sprites?.other?.["official-artwork"]?.front_default ? { url: data.sprites.other["official-artwork"].front_default } : undefined,
          fields: [
