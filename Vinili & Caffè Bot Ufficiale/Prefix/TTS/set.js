...
function parseBooleanState(raw) {
  const value = String(raw || "")
    .trim()
    .toLowerCase();
  if (["true", "on", "attivo", "attiva", "1", "si", "yes"].includes(value))
    return true;
  if (["false", "off", "disattivo", "disattiva", "0", "no"].includes(value))
    return false;
  return null;
}
    ...
    if (sub === "autojoin") {
      ...
      if (state == null) {
        await safeMessageReply(message, {
          content:
            "<:vegax:1443934876440068179> Usa: `+set autojoin true|false`",
