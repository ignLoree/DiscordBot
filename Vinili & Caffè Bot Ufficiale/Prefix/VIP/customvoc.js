...
function parseOptionalDuration(raw) {
  ...
  if (
    ["off", "none", "no", "perma", "permanent", "permanente"].includes(value)
  ) {
  ...
  if (!ms) {
    return {
      ...
      ms: null,
      error:
        "Durata non valida. Esempi: `14d`, `2w`, `2 settimane`, oppure `permanente`.",
    };
  }
...
async function updateCustomRoleTiming(
  guildId,
  userId,
  durationOption,
  extraSet = {},
  ...
  if (durationOption?.provided) {
    setData.expiresAt = durationOption.clear
      ? null
      : new Date(Date.now() + durationOption.ms);
  }
...
module.exports = {
  ...
  async execute(message, args = []) {
      ...
      await safeMessageReply(message, {
        embeds: [
          new EmbedBuilder()
            .setColor("Red")
            .setDescription(
              `<:vegax:1443934876440068179> ${durationOption.error}`,
            ),
        ],
      ...
      await safeMessageReply(message, {
        embeds: [
          new EmbedBuilder()
            .setColor("Red")
            .setDescription(
              "<:vegax:1443934876440068179> Non hai un ruolo personalizzato. Usa prima `+customrole create`.",
            ),
        ],
        allowedMentions: { repliedUser: false },
      });
      ...
      await safeMessageReply(message, {
        embeds: [
          new EmbedBuilder()
            .setColor("Red")
            .setDescription(
              "<:vegax:1443934876440068179> Il tuo ruolo personalizzato non esiste più. Ricrealo con `+customrole create`.",
            ),
        ],
        allowedMentions: { repliedUser: false },
      });
      ...
      await safeMessageReply(message, {
        embeds: [
          new EmbedBuilder()
            .setColor("Red")
            .setDescription(
              "<:vegax:1443934876440068179> Mi serve il permesso `Gestisci Canali` per creare la vocale privata.",
            ),
        ],
        allowedMentions: { repliedUser: false },
      });
      ...
      await safeMessageReply(message, {
        embeds: [
          new EmbedBuilder()
            .setColor("Red")
            .setDescription(
              "<:vegax:1443934876440068179> Categoria vocale privata non trovata o non valida.",
            ),
        ],
        allowedMentions: { repliedUser: false },
      });
    ...
    if (existing) {
      const updatedDoc=await updateCustomRoleTiming(message.guild.id,message.author.id,durationOption,{customVocChannelId:existing.id},);
      await safeMessageReply(message, {
        embeds: [
          buildVoicePanelEmbed(
            existing,
            customRole,
            updatedDoc || customRoleDoc,
            durationOption,
          ),
        ],
    ...
    const channel=await message.guild.channels.create({name:channelName,type:ChannelType.GuildVoice,parent:CUSTOM_VOICE_CATEGORY_ID,reason:`Private voice channel for ${message.author.tag}`,
      ...
      await safeMessageReply(message, {
        embeds: [
          new EmbedBuilder()
            .setColor("Red")
            .setDescription(
              "<:vegax:1443934876440068179> Non sono riuscito a creare la vocale privata.",
            ),
        ],
        allowedMentions: { repliedUser: false },
      });
    ...
    const updatedDoc=await updateCustomRoleTiming(message.guild.id,message.author.id,durationOption,{customVocChannelId:channel.id},);
