...
function parseUserDateInput(raw) {
    ...
    const words = value.match(/^(\d{1,2})\s+([a-zאטילעש]+)(?:\s+(\d{2,4}))?$/i);
    if (words) {
        ...
        switch (sub) {
            case 'request': {
                ...
                await safeEditReply(interaction, {
                    embeds: [
                        new EmbedBuilder()
                            .setDescription("<:vegacheckmark:1443666279058772028> La tua richiesta di pausa ט stata inviata all'High Staff")
                            .setColor('#6f4e37')
                    ],
                    flags: 1 << 6
                });
