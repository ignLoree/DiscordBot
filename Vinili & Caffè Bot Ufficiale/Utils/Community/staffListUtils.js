...
const ROLE_EMOJIS = {
  [IDs.roles.PartnerManager]: {
    emoji: "<:partnermanager:1443651916838998099>",
    number: "∞",
  },
...
async function resolveMessage(channel, client, guildId) {
  ...
  if (knownId) {
    const known =
      channel.messages.cache.get(knownId) ||
      (await channel.messages.fetch(knownId).catch(() => null));
