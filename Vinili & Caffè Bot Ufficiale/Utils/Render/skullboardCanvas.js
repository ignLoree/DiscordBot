...
module.exports = async function renderSkullboardCanvas({ avatarUrl, username, message, nameColor, createdAt, reply, roleIconUrl }) {
  ...
  if (replyLine) {
    ...
    ctx.lineCap = "round";
    ctx.lineJoin = "round";
    const connectorStartX = avatarX + avatarSize + 4;
    const connectorStartY = avatarY + avatarSize - 6;
    const connectorEndX = replyStartX - 6;
    const connectorEndY = replyY + 9;
    const radius = 8;
    ctx.beginPath();
    ctx.moveTo(connectorStartX, connectorStartY);
    ctx.lineTo(connectorStartX, connectorEndY - radius);
    ctx.arcTo(connectorStartX, connectorEndY, connectorStartX + radius, connectorEndY, radius);
