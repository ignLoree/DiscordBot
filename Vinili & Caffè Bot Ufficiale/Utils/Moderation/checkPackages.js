...
async function getFiles(dir) {
  ...
  const files = await Promise.all(entries.map((entry) => { const res = path.resolve(dir, entry.name); if (entry.isDirectory() && entry.name !== "node_modules") { return getFiles(res); } else if (entry.isFile() && res.endsWith(".js")) { return res; } return []; }),);
  return files.flat();
}
...
async function checkNodeVersion(client) {
  try {
    ...
    const compare = (a, b) => { for (let i = 0; i < 3; i += 1) { const diff = (a[i] || 0) - (b[i] || 0); if (diff !== 0) return diff; } return 0; };
...
async function checkAndInstallPackages(
  ...
  try {
    ...
    const missingPackages = [...requiredPackages].filter((pkg) => { if (ignorePackages.includes(pkg)) return false; if (installedPackages.has(pkg)) return false; const modulePath = path.join("node_modules", ...pkg.split("/")); return !fsSync.existsSync(modulePath); });
