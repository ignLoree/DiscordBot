...
async function handleSuggestionVote(interaction) {
            ...
            if (!hasSuggestionStaffAccess(interaction)) {
                await interaction.reply({
                    embeds: [
                        new EmbedBuilder()
                            .setColor('Red')
                            .setDescription('<:vegax:1443934876440068179> Questo controllo è riservato all\'High Staff.')
                    ],
                    flags: 1 << 6
                }).catch(() => {});
                return true;
            ...
            if (!Array.isArray(message.components) || message.components.length === 0) {
                await interaction.reply({
                    embeds: [
                        new EmbedBuilder()
                            .setColor('Yellow')
                            .setDescription('<:attentionfromvega:1443651874032062505> Questo suggerimento è già stato gestito.')
                    ],
                    flags: 1 << 6
                }).catch(() => {});
                return true;
    ...
    if (interaction.isModalSubmit && interaction.isModalSubmit()) {
        ...
        if (!hasSuggestionStaffAccess(interaction)) {
            await interaction.reply({
                embeds: [
                    new EmbedBuilder()
                        .setColor('Red')
                        .setDescription('<:vegax:1443934876440068179> Questo modulo è riservato all\'High Staff.')
                ],
                flags: 1 << 6
            }).catch(() => {});
            return true;
        ...
        if (!messageId || !['accept', 'reject'].includes(action)) {
            await interaction.reply({
                embeds: [new EmbedBuilder().setColor('Red').setDescription('<:vegax:1443934876440068179> Richiesta non valida.')],
                flags: 1 << 6
            }).catch(() => {});
            return true;
        }
        ...
        if (!reason) {
            await interaction.reply({
                embeds: [new EmbedBuilder().setColor('Red').setDescription('<:vegax:1443934876440068179> Devi inserire un motivo.')],
                flags: 1 << 6
            }).catch(() => {});
            return true;
        }
        ...
        if (!suggestionData) {
            await interaction.reply({
                embeds: [new EmbedBuilder().setColor('Red').setDescription('<:vegax:1443934876440068179> Suggerimento non trovato.')],
                flags: 1 << 6
            }).catch(() => {});
            return true;
        }
        ...
        const suggestionChannel =
            ...
            await interaction.guild.channels.fetch(IDs.channels.suggestions).catch(() => null);
        ...
        if (!suggestionMessage || !oldEmbed) {
            await interaction.reply({
                embeds: [new EmbedBuilder().setColor('Red').setDescription('<:vegax:1443934876440068179> Messaggio suggerimento non trovato.')],
                flags: 1 << 6
            }).catch(() => {});
            return true;
        }
        if (!Array.isArray(suggestionMessage.components) || suggestionMessage.components.length === 0) {
            await interaction.reply({
                embeds: [new EmbedBuilder().setColor('Yellow').setDescription('<:attentionfromvega:1443651874032062505> Questo suggerimento è già stato gestito.')],
                flags: 1 << 6
            }).catch(() => {});
            return true;
        }
        ...
        const resultEmbed = new EmbedBuilder()
            ...
            .setTitle(isAccept ? '<:pinnednew:1443670849990430750> Suggerimento Accettato!' : '<:pinnednew:1443670849990430750> Suggerimento Rifiutato!')
            ...
            .addFields({
                name: isAccept ? '<:pinnednew:1443670849990430750> Motivo:' : '<:attentionfromvega:1443651874032062505> Motivo del rifiuto:',
                value: reason
            });

        await suggestionMessage.edit({ embeds: [resultEmbed], components: [] }).catch(() => {});
        ...
        if (isAccept) {
            ...
            let levelsAwarded = 0;
            try {
                let expDoc = await ExpUser.findOne({ guildId, userId }).lean().catch(() => null);
                if (!expDoc) {
                    await ExpUser.create({ guildId, userId }).catch(() => {});
                    expDoc = await ExpUser.findOne({ guildId, userId }).lean().catch(() => null);
                }
                ...
                if (expToAdd > 0) {
                    await addExpWithLevel(interaction.guild, userId, expToAdd, false, false);
                    levelsAwarded = 5;
                }
            } catch (error) {
            ...
            if (supportersChannel) {
                const thanksText = levelsAwarded > 0
        ...
        const suggestionAuthor = await interaction.client.users.fetch(suggestionData.AuthorID).catch(() => null);
        if (suggestionAuthor) {
