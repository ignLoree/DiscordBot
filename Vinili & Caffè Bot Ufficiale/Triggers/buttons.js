...
const MONO_GUILD_DENIED_TEXT="Questo bot ÃƒÆ’Ã‚¨ utilizzabile solo sul server principale di Vinili & CaffÃƒ¨.";

async function handleStaffButtons(interaction) {
  if (!interaction.isButton()) return false;

  if (interaction.customId == "sanzioni") {
    const embed=new EmbedBuilder().setImage(DIVIDER_URL).setColor("#6f4e37").setDescription(`<:reportmessage:1443670575376765130> Ogni staffer per sanzionare dovrÃƒÆ’Ã‚  __seguire__ <#1442569243626307634>, chi non lo farÃƒÆ’Ã‚  **riceverÃƒÆ’Ã‚ ** una __valutazione negativa__.

            > <a:VC_Arrow:1448672967721615452> **__LIMITI SETTIMANALI SULLE SANZIONI__**
            <:dot:1443660294596329582> Ogni <@&1442568901887000618> dovrÃƒÆ’Ã‚  __eseguire__ almeno: **\`3 sanzioni\`**
            <:dot:1443660294596329582> Ogni <@&1442568897902678038> dovrÃƒÆ’Ã‚  __eseguire__ almeno: **\`4 sanzioni\`**
            <:dot:1443660294596329582> Ogni <@&1442568896237277295> dovrÃƒÆ’Ã‚  __eseguire__ almeno: **\`4 sanzioni\`**

            > Chi __rispetterÃƒÆ’Ã‚ __ questi limiti riceverÃƒÆ’Ã‚  **una valutazione positiva**.`);
    await interaction.reply({ embeds: [embed], flags: 1 << 6 }).catch(() => null);
    return true;
  }

  if (interaction.customId == "warnstaff") {
    const embed=new EmbedBuilder().setImage(DIVIDER_URL).setColor("#6f4e37").setDescription(`<:banhammer:1443651875441217639>  I **warn staff** vengono __assegnati__ dopo **3 valutazioni negative**. Raggiunti i \`2\` **warn staff** si verrÃƒ  depexati al ruolo precedente. **__(Per i Mod sarÃƒÆ’Ã‚  depex completo)__**

                    > L'<@&1442568894349840435> puÃƒÆ’Ã‚² decidere di grazie qualcuno al secondo warn, ma in caso di **terzo warn** lo staffer verrÃƒ  depexato **__completamente__**
  ...
  if (interaction.customId == "valutazioni") {
    const embed=new EmbedBuilder().setImage(DIVIDER_URL).setColor("#6f4e37").setDescription(`<a:1370everythingisstable:1444006799643508778> **__VALUTAZIONI POSITIVE__**
                    <a:questionexclaimanimated:1443660299994533960> Le **valutazioni positive** aumentano la possibilitÃƒÆ’Ã‚  di essere **pexati** e si possono **ottenere** generando un'__attivitÃƒ __ **superiore** a quella richiesta nei _limiti settimanali_ o facendo qualcosa per dare un **vantaggio** al __server__.

                    > Le **valutazioni positive** si possono **__scambiare__** per dei giorni in piÃƒÆ’Ã‚¹ di **pausa**.

                    <a:laydowntorest:1444006796661358673> **__VALUTAZIONI NEGATIVE__**
                    > Le **valutazioni negative** diminuiscono la possibilitÃƒÆ’Ã‚  di essere **pexati** e si ottengono **non completando** i _limiti settimanali_ o facendo qualcosa di _nocivo_ per il **server**.

                    > Le **valutazioni negative** possono essere **__rimosse__** completando compiti extra assegnati dall'<@&1442568894349840435> o rinunciando a _almeno_ \`3 o piÃƒÆ’Ã‚¹ valutazioni positive\` in base al motivo per cui ÃƒÆ’Ã‚¨ stata assegnata la valutazione. `);
    await interaction.reply({ embeds: [embed], flags: 1 << 6 }).catch(() => null);
    return true;
  }

  if (interaction.customId == "pause") {
    const embed=new EmbedBuilder().setImage(DIVIDER_URL).setColor("#6f4e37").setDescription(`<:Clock:1330530065133338685> Vinili & CaffÃƒ¨ presenta un **sistema** di **\`pause\`** _sofisticato_, infatti ÃƒÆ’Ã‚¨ tutto __organizzato__ per **garantire** al meglio l'__attivitÃƒ __ del server.
                ...
                <a:loading:1443934440614264924> In un anno si possono chiedere __massimo__ **\`60 giorni\`** di pausa che si possono usufruire in __tutti__ i **12 mesi** dell'anno. Gli <@&1442568895251611924> avranno **__\`5\`__** giorni in piÃƒÆ’Ã‚¹.

                > Nei giorni **festivi** avrete anche dei giorni in piÃƒÆ’Ã‚¹ **ulteriori** al __normale mese__:
            ...
            > Naturalmente per **garantire** al meglio l'attivitÃƒ  del server ci sono dei __limiti__ di staffer che possono essere in pausa nello __stesso periodo__
            ...
            <:attentionfromvega:1443651874032062505> Potrai chiedere __una pausa__ **ogni mese**. Se quest'ultima cade in \`2\` **__mesi diversi__**, verrÃƒ  contato il mese in cui viene **chiesta** la __pausa__, a patto che non superi i primi __\`5\` giorni__ dell'altro mese. Per chiedere un'altra pausa dovrai aspettare almeno **__\`1 settimana\`__**

            <:infoglowingdot:1443660296823767110> Gli __\`Helper\`__ potranno richiedere una __pausa__ nella loro prima settimana **solo** in per **__problemi personali__ o __familiari gravi__**. Se l'<@&1442568894349840435> viene a conoscenza di un **__falso__ motivo** per usare una pausa, **in qualsiasi circostanza**, si verrÃƒ  **__depexati__** all'**__istante__**.

            > <:banhammer:1443651875441217639> Se l'<@&1442568894349840435> verrÃƒ  a conoscenza di uno **staffer** __in pausa__ ma **attivo** in un **altro server** nel periodo di tempo della pausa, **toglierÃƒÆ’Ã‚ ** la pausa e **sanzionerÃƒÆ’Ã‚ ** lo staffer.`);
    await interaction.reply({ embeds: [embed], flags: 1 << 6 }).catch(() => null);
    return true;
  }

  if (interaction.customId == "limiti") {
    const embed=new EmbedBuilder().setImage(DIVIDER_URL).setColor("#6f4e37").setDescription(`<:infoglowingdot:1443660296823767110> I **\`limiti settimanali\`** sono dei **messaggi** e delle **ore di vocali** che entro una __settimana__ si devono __raggiungere__.
            ...
            > <:attentionfromvega:1443651874032062505> verrÃƒ  **valutato** anche il **modo** in cui questi __limiti__ vengono raggiunti, ovvero se lo **staffer** ÃƒÆ’Ã‚¨ stato costante o no. `);
    await interaction.reply({ embeds: [embed], flags: 1 << 6 }).catch(() => null);
    return true;
  }
  ...
  if (interaction.customId == "regolamento") {
    const embed=new EmbedBuilder().setImage(DIVIDER_URL).setColor("#6f4e37").setDescription(`<:dot:1443660294596329582> **Rispettare** le regole (<#1442569111119990887>) del server;
            ...
            <:dot:1443660294596329582> Se si ÃƒÆ’Ã‚¨ in una **vocale pubblica** da __mutati__ siete **obbligati** a scrivere in <#1442569130573303898>;

            <:dot:1443660294596329582> __Non__ **floodare**, **spammare** e **usare bot** per completare i **\`limiti settimanali testuali\`**

            <:dot:1443660294596329582> __Non__ passare la maggior parte del **tempo** nei **canali vocali privati**, poichÃƒÆ’Ã‚© non vengono **conteggiati** al fine dei __limiti settimanali__

            <:dot:1443660294596329582> __Non__ **stare da soli** in una __vocale pubblica__ se in un'altra vi ÃƒÆ’Ã‚¨ giÃƒÆ’Ã‚  un altro **staffer** da solo. Inoltre, almeno uno dei due **deve** essere __smutato__`);
    await interaction.reply({ embeds: [embed], flags: 1 << 6 }).catch(() => null);
    return true;
  }
  ...
  if (interaction.customId == "generalimoderazione") {
    const embed=new EmbedBuilder().setImage(DIVIDER_URL).setColor("#6f4e37").setDescription(`<:rules:1443307208543703131> **__Regola \`1.1\`__**
                ...
                <:VC_DoubleReply:1468713981152727120> 1Ãƒâ€šÃ‚° Sanzione: **Richiamo verbale**
                <:VC_Reply:1468262952934314131> 2Ãƒâ€šÃ‚° Sanzione: **Mute __2h__**

                <:rules:1443307208543703131> **__Regola \`1.3\`__**
                <:VC_DoubleReply:1468713981152727120> 1Ãƒâ€šÃ‚° Sanzione: **Richiamo verbale**
                <:VC_Reply:1468262952934314131> 2Ãƒâ€šÃ‚° Sanzione: **Mute __3h__**
                <:VC_Reply:1468262952934314131> 3Ãƒâ€šÃ‚° Sanzione: **Mute __6h__**
                <:VC_Reply:1468262952934314131> 4Ãƒâ€šÃ‚° Sanzione: **Mute __18h__**

                <:rules:1443307208543703131> **__Regola \`1.4\`__**
                <:VC_DoubleReply:1468713981152727120> 1Ãƒâ€šÃ‚° Sanzione: **Mute __12h__**
                <:VC_Reply:1468262952934314131> 2Ãƒâ€šÃ‚° Sanzione: **Ban**
  ...
  if (interaction.customId == "testualimoderazione") {
    const embed=new EmbedBuilder().setImage(DIVIDER_URL).setColor("#6f4e37").setDescription(`<:rules:1443307208543703131> **__Regola \`2.1\`__**
                <:VC_DoubleReply:1468713981152727120> 1Ãƒâ€šÃ‚° Sanzione: **Mute __18h__**
                <:VC_Reply:1468262952934314131> 2Ãƒâ€šÃ‚° Sanzione: **Ban**

                <:rules:1443307208543703131> **__Regola \`2.2\`__**
                <:VC_DoubleReply:1468713981152727120> 1Ãƒâ€šÃ‚° Sanzione: **Richiamo verbale**
                <:VC_Reply:1468262952934314131> 2Ãƒâ€šÃ‚° Sanzione: **Warn**
                <:VC_Reply:1468262952934314131> 3Ãƒâ€šÃ‚° Sanzione: **Mute __12h__**
                <:VC_Reply:1468262952934314131> 4Ãƒâ€šÃ‚° Sanzione: **Mute __18h__**
                <:VC_Reply:1468262952934314131> 5Ãƒâ€šÃ‚° Sanzione: **Mute __24h__**
                <:VC_Reply:1468262952934314131> 6Ãƒâ€šÃ‚° Sanzione: **Ban**
  ...
  if (interaction.customId == "vocalimoderazione") {
    const embed=new EmbedBuilder().setImage(DIVIDER_URL).setColor("#6f4e37").setDescription(`<:rules:1443307208543703131> **__Regola \`3.1\`__**
                <:VC_DoubleReply:1468713981152727120> 1Ãƒâ€šÃ‚° Sanzione: **Richiamo verbale**
                <:VC_Reply:1468262952934314131> 2Ãƒâ€šÃ‚° Sanzione: **Mute __16h__**
                <:VC_Reply:1468262952934314131> 3Ãƒâ€šÃ‚° Sanzione: **Ban**

                <:rules:1443307208543703131> **__Regola \`3.2\`__**
                <:VC_DoubleReply:1468713981152727120> 1Ãƒâ€šÃ‚° Sanzione: **Richiamo verbale**
                <:VC_Reply:1468262952934314131> 2Ãƒâ€šÃ‚° Sanzione: **Mute __12h__**
                <:VC_Reply:1468262952934314131> 3Ãƒâ€šÃ‚° Sanzione: **Mute __18h__**
                <:VC_Reply:1468262952934314131> 4Ãƒâ€šÃ‚° Sanzione: **Mute __24h__**
                <:VC_Reply:1468262952934314131> 5Ãƒâ€šÃ‚° Sanzione: **Ban**

                <:rules:1443307208543703131> **__Regola \`3.3\`__**
                <:VC_DoubleReply:1468713981152727120> 1Ãƒâ€šÃ‚° Sanzione: **Richiamo verbale**
                <:VC_Reply:1468262952934314131> 2Ãƒâ€šÃ‚° Sanzione: **Mute __6h__**
                <:VC_Reply:1468262952934314131> 3Ãƒâ€šÃ‚° Sanzione: **Mute __12h__**
                <:VC_Reply:1468262952934314131> 4Ãƒâ€šÃ‚° Sanzione: **Mute __24h__**
                <:VC_Reply:1468262952934314131> 5Ãƒâ€šÃ‚° Sanzione: **Mute __48h__**

                <:rules:1443307208543703131> **__Regola \`3.4\`__**
                <:VC_DoubleReply:1468713981152727120> 1Ãƒâ€šÃ‚° Sanzione: **Richiamo verbale**
                <:VC_Reply:1468262952934314131> 2Ãƒâ€šÃ‚° Sanzione: **Mute __3h__**
                <:VC_Reply:1468262952934314131> 3Ãƒâ€šÃ‚° Sanzione: **Mute __6h__**
                <:VC_Reply:1468262952934314131> 4Ãƒâ€šÃ‚° Sanzione: **Mute __12h__**
                <:VC_Reply:1468262952934314131> 5Ãƒâ€šÃ‚° Sanzione: **Ban**
                ...
                <:VC_Reply:1468262952934314131> 2Ãƒâ€šÃ‚° Sanzione: **Mute __3h__**
                <:VC_Reply:1468262952934314131> 3Ãƒâ€šÃ‚° Sanzione: **Mute __12h__**
                <:VC_Reply:1468262952934314131> 4Ãƒâ€šÃ‚° Sanzione: **Ban**
                ...
                <:VC_DoubleReply:1468713981152727120> 1Ãƒâ€šÃ‚° Sanzione: **Richiamo verbale**
                <:VC_Reply:1468262952934314131> 2Ãƒâ€šÃ‚° Sanzione: **Mute __4h__**
                <:VC_Reply:1468262952934314131> 3Ãƒâ€šÃ‚° Sanzione: **Mute __12h__**
                <:VC_Reply:1468262952934314131> 4Ãƒâ€šÃ‚° Sanzione: **Mute __18h__**
                <:VC_Reply:1468262952934314131> 5Ãƒâ€šÃ‚° Sanzione: **Ban**`);
    await interaction.reply({ embeds: [embed], flags: 1 << 6 }).catch(() => null);
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (
      ...
      if (interaction.user.id !== targetId) {
        const denied=new EmbedBuilder().setColor("#e74c3c").setTitle("<:vegax:1443934876440068179> Accesso negato").setDescription("Solo l'autore della citazione puÃƒÆ’Ã‚² rimuoverla.");
        return interaction.reply({ embeds: [denied], flags: 1 << 6 });
      }
      const now = new Date();
      const dateStr = now.toLocaleDateString("it-IT");
      const timeStr=now.toLocaleTimeString("it-IT",{hour:"2-digit",minute:"2-digit",});
      const removedEmbed=new EmbedBuilder().setImage(DIVIDER_URL).setColor("#6f4e37").setTitle("\u{1F5D1}\uFE0F Citazione rimossa").setDescription("Questa citazione \u00E8 stata rimossa dall'autore.").addFields({name:"Rimossa da",value:`<@${interaction.user.id}>`,
            inline: true,
          },
          {
            name: "Data rimozione",
    ...
    if (interaction.customId == "metodi") {
      const embeds=[new EmbedBuilder().setImage(DIVIDER_URL).setColor("#6f4e37").setDescription(`<:Money:1330544713463500970> Per effettuare una sponsor con __Vinili & CaffÃƒ¨__ ci sono due modalitÃƒÆ’Ã‚ : **pagando** oppure esponendo una **collaborazione** in un <#${IDs.channels.ticket}>\`Terza Categoria\`.

                    <:dot:1443660294596329582> **Ãƒ¢Ã¢â‚¬Å¡Ã‚¬1,50** <a:VC_Arrow:1448672967721615452> sponsor per **2** settimane
                    <:dot:1443660294596329582> **Ãƒ¢Ã¢â‚¬Å¡Ã‚¬3** <a:VC_Arrow:1448672967721615452> sponsor per **1** mese
                    <:dot:1443660294596329582> **Ãƒ¢Ã¢â‚¬Å¡Ã‚¬5** <a:VC_Arrow:1448672967721615452> sponsor **lifetime**`),];
      await interaction.reply({ embeds: [embeds[0]], flags: 1 << 6 }).catch(() => null);
    }
    if (interaction.customId == "ping") {
      const embeds=[new EmbedBuilder().setImage(DIVIDER_URL).setColor("#6f4e37").setDescription(`<:Discord_Mention:1329524304790028328> I **ping** variano in base al __numero__ di **membri** del server.
    ...
    if (interaction.customId == "info_rules") {
      ...
      const generalEmbed=new EmbedBuilder().setImage(DIVIDER_URL).setColor("#6f4e37").setDescription(`
        ...
        <:VC_Reply:1468262952934314131> ÃƒË† __vietato__ **auto-promuoversi**.

        <:rules:1443307208543703131> **\`REGOLA GENERALE 1.5\`**
        <:VC_Reply:1468262952934314131> ÃƒË† __vietato__ **uscire** e **rientrare** continuamente dal server.`);
      const textEmbed=new EmbedBuilder().setImage(DIVIDER_URL).setColor("#6f4e37").setDescription(`<:rules:1443307208543703131> **\`REGOLA TESTUALE 2.1\`**
                <:VC_Reply:1468262952934314131> ÃƒË† __vietato__ inviare **file** **gore**, **NSFW** o **dati sensibili** di un utente.

        <:rules:1443307208543703131> **\`REGOLA TESTUALE 2.2\`**
        <:VC_Reply:1468262952934314131> ÃƒË† __vietato__ avere **comportamenti toxic** o **troll** che conducono al flame.

        <:rules:1443307208543703131> **\`REGOLA TESTUALE 2.3\`**
        <:VC_Reply:1468262952934314131> ÃƒË† __vietato__ inviare **link** contenenti virus, grabber, sponsor o social.

        <:rules:1443307208543703131> **\`REGOLA TESTUALE 2.4\`**
        <:VC_Reply:1468262952934314131> ÃƒË† __vietato__ inviare **flood** o **Wall Of Text** che intasano la chat.

        <:rules:1443307208543703131> **\`REGOLA TESTUALE 2.5\`**
        <:VC_Reply:1468262952934314131> ÃƒË† __vietato__ abusare di **parolacce**, **bestemmie** e ogni tipo di **insulto** a **divinitÃƒÆ’Ã‚ **.`);
      const voiceEmbed=new EmbedBuilder().setImage(DIVIDER_URL).setColor("#6f4e37").setDescription(`<:rules:1443307208543703131> **\`REGOLA VOCALE 3.1\`**
                <:VC_Reply:1468262952934314131> ÃƒË† __vietato__ mostrare contenuti **gore**, **NSFW** o **dati sensibili** di un utente.

        <:rules:1443307208543703131> **\`REGOLA VOCALE 3.2\`**
        <:VC_Reply:1468262952934314131> ÃƒË† __vietato__ avere **comportamenti toxic** o **troll** che conducono al flame.

        <:rules:1443307208543703131> **\`REGOLA VOCALE 3.3\`**
        <:VC_Reply:1468262952934314131> ÃƒË† __vietato__ **disconnettere il bot** o cambiare musica mentre un utente sta ascoltando una canzone tramite il bot.

        <:rules:1443307208543703131> **\`REGOLA VOCALE 3.4\`**
        <:VC_Reply:1468262952934314131> ÃƒË† __vietato__ utilizzare **SoundBoard** o qualunque tipo di **VoiceChanger**.

        <:rules:1443307208543703131> **\`REGOLA VOCALE 3.5\`**
        <:VC_Reply:1468262952934314131> ÃƒË† __vietato__ **urlare** o fare **errori** col microfono.

        <:rules:1443307208543703131> **\`REGOLA VOCALE 3.6\`**
        <:VC_Reply:1468262952934314131> ÃƒË† __vietato__ abusare di **parolacce** e **bestemmie** e ogni tipo di **insulto** a **divinitÃƒÆ’Ã‚ **.

        <:rules:1443307208543703131> **\`REGOLA VOCALE 3.7\`**
        <:VC_Reply:1468262952934314131> ÃƒË† __vietato__ **uscire** e **rientrare** continuamente dalle vocali.`);
    ...
    if (interaction.customId == "info_donations") {
      const donations=new EmbedBuilder().setImage(DIVIDER_URL).setColor("#6f4e37").setAuthor({name:`Supporta il server con l'acquisto di un VIP o una donazione!`,iconURL:`https://emoji.gg/emoji/480166-coffee`,}).setDescription([`Usiamo i soldi donati per portare eventi con premi migliori, come Discord Nitro o gift card, e per cercare collaborazioni interessanti per voi utenti.`,"Le donazioni sono completamente volontarie e contribuiscono alla nostra crescita. In anticipo, grazie.","","**Vuoi acquistare direttamente tutti i vantaggi permanenti e differenti dal VIP?**","\`5,00Ãƒ¢Ã¢â‚¬Å¡Ã‚¬\`","<:dot:1443660294596329582> Il ruolo <@&1442568916114346096> **permanente**","<:dot:1443660294596329582> PossibilitÃƒ  di allegare link e immagini in chat","<:dot:1443660294596329582> PossibilitÃƒ  di usare le soundboard di altri server","<:dot:1443660294596329582> PossibilitÃƒ  di mandare stickers ed emoji di altri server","<:dot:1443660294596329582> Bypass dei requisiti nei giveaway","<:dot:1443660294596329582> PossibilitÃƒ  di cambiare il tuo nickname","<:dot:1443660294596329582> Sblocchi i COLORI PLUS (gradienti) su <#1469429150669602961>","<:dot:1443660294596329582> Sblocchi il comando \`+quote\`","<:dot:1443660294596329582> X3 EXP boost",].join("\n"),).setImage(DIVIDER_URL);

      const vip=new EmbedBuilder().setImage(DIVIDER_URL).setColor("#6f4e37").setDescription([`**Vuoi sostenere il server?** Acquista il VIP e riscatta tutti i vantaggi che hai sbloccato qui nel server! <a:VC_HeartsBlue:1468686100045369404>`,"","ACQUISTA IL <@&1442568950805430312> <a:VC_HeartsPink:1468685897389052008>","\`10,00Ãƒ¢Ã¢â‚¬Å¡Ã‚¬\`Ãƒ¢Ã‹Å“Ã¢â‚¬ ","","<:sparkledred:1470064814502973591> Sbloccherai:","<:dot:1443660294596329582> Permesso di allegare link e immagini in chat","<:dot:1443660294596329582> Permesso di usare soundboard di altri server","<:dot:1443660294596329582> PossibilitÃƒ  di mandare stickers ed emoji di altri server","<:dot:1443660294596329582> Bypass dei requisiti nei giveaway","<:dot:1443660294596329582> PossibilitÃƒ  di creare una vocale privata per te e i tuoi amici","<:dot:1443660294596329582> Reazioni al messaggio quando viene @menzionato in chat (max. 3 reazioni).","<:dot:1443660294596329582> Potrai crearti un ruolo personalizzato con colore GRADIENTE scelto da te.","<:dot:1443660294596329582> X4 EXP boost","","<:attentionfromvega:1443651874032062505> DISCLAIMER / I soldi non sono rimborsabili, essendo volontari.",'I vantaggi segnati in "Ã¢Ëœâ€ " vengono rimossi qualora l\'utente violi il regolamento, porti un immagine negativa al server o sia inattivo per mesi.',].join("\n"),).setImage(DIVIDER_URL);

      const ticket=new EmbedBuilder().setImage(DIVIDER_URL).setColor("#6f4e37").setTitle(`Acquista ora!`).setDescription(`<:blueflash:1470064803157643468> Apri un ticket nella **terza categoria** su: <#1442569095068254219>.Ã¡Å¸`,);
      await interaction.reply({
        embeds: [donations, vip, ticket],
        flags: 1 << 6,
      }).catch(() => null);
    }
    const sendUpdatedView=async(payload) => {const msgFlags=interaction.message?.flags;const isEphemeralSource=Boolean((typeof msgFlags?.has==="function"&&msgFlags.has(1<<6))||(typeof msgFlags?.bitfield==="number"&&(msgFlags.bitfield&(1<<6))!==0)||(typeof msgFlags==="number"&&(msgFlags&(1<<6))!==0),);if(isEphemeralSource){return interaction.update(payload).catch(async() => {return interaction.reply({...payload,flags:1<<6}).catch(() => {});});}return interaction.reply({...payload,flags:1<<6}).catch(() => {});};if(interaction.customId=="info_verifica"){const verifyEmbed=new EmbedBuilder().setImage(DIVIDER_URL).setColor("#6f4e37").setTitle("<a:VC_Verified:1448687631109197978>Ã£Æ’»**__Verificati per ottenere vantaggi unici__**",).setDescription([`Per verificare il tuo profilo dovrai andare in <#${IDs.channels.ticket}> e selezionare la terza categoria`,
            ...
            "",
            "<:sparkledred:1470064814502973591>Ã£Æ’»Ruolo <@&1469040179799920801> o <@&1469040190730408018> con badge speciale",
            "<:moon:1470064812615667827>Ã£Æ’»Permesso di allegare immagini e link in chat",
            "<:pinkstar:1470064804835229768>Ã£Æ’»Permesso di scrivere in <#1470029899740873029>",
          ].join("\n"),
        );
    ...
    const buildBoostLevelsPayload=() => {const boostEmbed=new EmbedBuilder().setImage(DIVIDER_URL).setColor("#6f4e37").setTitle("<a:VC_HeartWhite:1448673535253024860>Ã£Æ’»**__Potenzia il server e sblocca vantaggi unici__**",).setDescription(["Un modo per sostenere il server Ãƒ¨ potenziarlo: se hai un Nitro Boost (quello da 9,99Ãƒ¢Ã¢â‚¬Å¡Ã‚¬) hai a disposizione 2 potenziamenti che puoi utilizzare in qualunque server tu voglia. Se deciderai di potenziare noi, __Vinili & CaffÃƒ¨__, sbloccherai un sacco di vantaggi.","**Non sai cos'ÃƒÆ’Ã‚¨ Discord Nitro?** <:link:1470064815899803668> [Scoprilo qui](<https://discord.com/nitro>).","",`<:sparkledred:1470064814502973591>Ã£Æ’»Ruolo <@&${IDs.roles.ServerBooster}> con badge speciale`,
            "<:moon:1470064812615667827>Ã£Æ’»Permesso di allegare immagini e link in chat",
            "<:pinkstar:1470064804835229768>Ã£Æ’»Permesso di mandare emoji e adesivi di altri server",
            "<:sparkle:1470064801811140866>Ã£Æ’»Permesso di usare le Soundboard del server",
            "<:blueflash:1470064803157643468>Ã£Æ’»PossibilitÃƒ  di creare un **ruolo personalizzato** e una **vocale privata personalizzata**",
            "<a:reddiamond:1443652837346377841>Ã£Æ’»Sblocchi il comando \`+quote\`",
            "<:exp:1470067108543987846>Ã£Æ’»X2 EXP Boost",
          ].join("\n"),
        )
        .setImage(DIVIDER_URL);

      const howtoEmbed=new EmbedBuilder().setImage(DIVIDER_URL).setColor("#6f4e37").setTitle("<:nitroboost:1470064881674883326>Ã£Æ’»**__Come creare ruolo personalizzato e vocale privata__**",).setDescription([`Usa\`+customrole create\` in <#${IDs.channels.commands}>per creare e configurare il ruolo.`,
    ...
    const buildLevelsPayload=() => {const levelEmbed=new EmbedBuilder().setImage(DIVIDER_URL).setColor("#6f4e37").setTitle("<:exp:1470067108543987846>Ã£Æ’»**__Sali di livello e sblocca vantaggi sempre migliori__**",).setDescription(["I livelli nel server rappresentano la tua attivitÃƒ : scrivendo in chat testuale e stando nei canali vocali, guadagnerai esperienza che verrÃƒ  aggiunta al tuo livello globale. ","Una volta raggiunta una certa somma di esperienza, farai un **level up**!","",`<:dot:1443660294596329582> Per __vedere i tuoi exp__ e le tue statistiche, usa i comandi: \`+rank\` in  <#${IDs.channels.commands}>.`,
            "",
            "<:VC_Dot:1443932948599668746> Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€¹â€ Ã¢â€¹…Ã¢Ëœâ€ Ã¢â€¹…Ã¢â€¹â€ Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ à­¨Ã¢â„¢¡à­§ Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€¹â€ Ã¢â€¹…Ã¢Ëœâ€ Ã¢â€¹…Ã¢â€¹â€ Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ à­¨Ã¢â„¢¡à­§ Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€¹â€ Ã¢â€¹…Ã¢Ëœâ€ Ã¢â€¹…Ã¢â€¹â€ Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ <:VC_Dot:1443932948599668746>",
            "",
        ...
        .addFields(
          ...
          },
          {
            ...
            value: [
              `<@&${IDs.roles.Level20}>`,
              "à¥¯ Tutte le ricompense precedenti",
              "<:VC_DoubleReply:1468713981152727120> PossibilitÃƒ  di aggiungere reazioni ai messaggi in chat",
            ].join("\n"),
            inline: true,
          },
          {
          ...
          },
          {
            ...
            value: [
              `<@&${IDs.roles.Level50}>`,
              "à¥¯ Tutte le ricompense precedenti",
              `<:VC_DoubleReply:1468713981152727120>PossibilitÃƒ  di usare i**colori PLUS**su<#${IDs.channels.ruoliColori}>`,
              "<:VC_DoubleReply:1468713981152727120> PossibilitÃƒ  di creare un ruolo __personalizzato PERMANENTE__ e un canale vocale privato __personalizzato PERMANENTE__.",
