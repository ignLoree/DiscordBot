...
async function handleStaffButtons(interaction) {
  ...
  if (interaction.customId == "sanzioni") {
    const embed=new EmbedBuilder().setImage(DIVIDER_URL).setColor("#6f4e37").setDescription(`<:reportmessage:1443670575376765130> Ogni staffer per sanzionare dovrà __seguire__ <#1442569243626307634>, chi non lo farà **riceverà** una __valutazione negativa__.
  ...
  if (interaction.customId == "warnstaff") {
    const embed=new EmbedBuilder().setImage(DIVIDER_URL).setColor("#6f4e37").setDescription(`<:banhammer:1443651875441217639>  I **warn staff** vengono __assegnati__ dopo **3 valutazioni negative**. Raggiunti i \`2\` **warn staff** si verrà depexati al ruolo precedente. **__(Per i Mod sarà depex completo)__**
  ...
  if (interaction.customId == "pause") {
    const embed=new EmbedBuilder().setImage(DIVIDER_URL).setColor("#6f4e37").setDescription(`<:Clock:1330530065133338685> Vinili & Caffè presenta un **sistema** di **\`pause\`** _sofisticato_, infatti è tutto __organizzato__ per **garantire** al meglio l'__attività__ del server.
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (
      ...
      const removedEmbed=new EmbedBuilder().setImage(DIVIDER_URL).setColor("#6f4e37").setTitle("🗑️ Citazione rimossa").setDescription("Questa citazione è stata rimossa dall'autore.").addFields({name:"Rimossa da",value:`<@${interaction.user.id}>`,
            inline: true,
          },
          {
            name: "Data rimozione",
    ...
    if (interaction.customId == "metodi") {
      const embeds=[new EmbedBuilder().setImage(DIVIDER_URL).setColor("#6f4e37").setDescription(`<:Money:1330544713463500970> Per effettuare una sponsor con __Vinili & Caffè__ ci sono due modalità: **pagando** oppure esponendo una **collaborazione** in un <#${IDs.channels.ticket}>\`Terza Categoria\`.
    ...
    if (interaction.customId == "info_donations") {
      const donations=new EmbedBuilder().setImage(DIVIDER_URL).setColor("#6f4e37").setAuthor({name:`Supporta il server con l'acquisto di un VIP o una donazione!`,iconURL:`https://emoji.gg/emoji/480166-coffee`,}).setDescription([`Usiamo i soldi donati per portare eventi con premi migliori, come Discord Nitro o gift card, e per cercare collaborazioni interessanti per voi utenti.`,"Le donazioni sono completamente volontarie e contribuiscono alla nostra crescita. In anticipo, grazie.","","**Vuoi acquistare direttamente tutti i vantaggi permanenti e differenti dal VIP?**","\`5,00€\`","<:dot:1443660294596329582> Il ruolo <@&1442568916114346096> **permanente**","<:dot:1443660294596329582> Possibilità di allegare link e immagini in chat","<:dot:1443660294596329582> Possibilità di usare le soundboard di altri server","<:dot:1443660294596329582> Possibilità di mandare stickers ed emoji di altri server","<:dot:1443660294596329582> Bypass dei requisiti nei giveaway","<:dot:1443660294596329582> Possibilità di cambiare il tuo nickname","<:dot:1443660294596329582> Sblocchi i COLORI PLUS (gradienti) su <#1469429150669602961>","<:dot:1443660294596329582> Sblocchi il comando \`+quote\`","<:dot:1443660294596329582> X3 EXP boost",].join("\n"),).setImage(DIVIDER_URL);

      const vip=new EmbedBuilder().setImage(DIVIDER_URL).setColor("#6f4e37").setDescription([`**Vuoi sostenere il server?** Acquista il VIP e riscatta tutti i vantaggi che hai sbloccato qui nel server! <a:VC_HeartsBlue:1468686100045369404>`,"","ACQUISTA IL <@&1442568950805430312> <a:VC_HeartsPink:1468685897389052008>","\`10,00€\`☆","","<:sparkledred:1470064814502973591> Sbloccherai:","<:dot:1443660294596329582> Permesso di allegare link e immagini in chat","<:dot:1443660294596329582> Permesso di usare soundboard di altri server","<:dot:1443660294596329582> Possibilità di mandare stickers ed emoji di altri server","<:dot:1443660294596329582> Bypass dei requisiti nei giveaway","<:dot:1443660294596329582> Possibilità di creare una vocale privata per te e i tuoi amici","<:dot:1443660294596329582> Reazioni al messaggio quando viene @menzionato in chat (max. 3 reazioni).","<:dot:1443660294596329582> Potrai crearti un ruolo personalizzato con colore GRADIENTE scelto da te.","<:dot:1443660294596329582> X4 EXP boost","","<:attentionfromvega:1443651874032062505> DISCLAIMER / I soldi non sono rimborsabili essendo volontari.",'I vantaggi segnati in "☆" vengono rimossi qualora l\'utente violi il regolamento, porti un immagine negativa al server o sia inattivo per mesi.',].join("\n"),).setImage(DIVIDER_URL);

      const ticket=new EmbedBuilder().setImage(DIVIDER_URL).setColor("#6f4e37").setTitle(`Acquista ora!`).setDescription(`<:blueflash:1470064803157643468> Apri un ticket nella **terza categoria** su: <#1442569095068254219>.ᐟ`,);
      await interaction.reply({
        embeds: [donations, vip, ticket],
        flags: 1 << 6,
      }).catch(() => null);
    }
    const sendUpdatedView=async(payload) => {const msgFlags=interaction.message?.flags;const isEphemeralSource=Boolean((typeof msgFlags?.has==="function"&&msgFlags.has(1<<6))||(typeof msgFlags?.bitfield==="number"&&(msgFlags.bitfield&(1<<6))!==0)||(typeof msgFlags==="number"&&(msgFlags&(1<<6))!==0),);if(isEphemeralSource){return interaction.update(payload).catch(async() => {return interaction.reply({...payload,flags:1<<6}).catch(() => {});});}return interaction.reply({...payload,flags:1<<6}).catch(() => {});};if(interaction.customId=="info_verifica"){const verifyEmbed=new EmbedBuilder().setImage(DIVIDER_URL).setColor("#6f4e37").setTitle("<a:VC_Verified:1448687631109197978>・**__Verificati per ottenere vantaggi unici__**",).setDescription([`Per verificare il tuo profilo dovrai andare in <#${IDs.channels.ticket}>e selezionare la terza categoria`,
    ...
    const buildBoostLevelsPayload=() => {const boostEmbed=new EmbedBuilder().setImage(DIVIDER_URL).setColor("#6f4e37").setTitle("<a:VC_HeartWhite:1448673535253024860>・**__Potenzia il server e sblocca vantaggi unici__**",).setDescription(["Un modo per sostenere il server è potenziarlo: se hai un Nitro Boost (quello da 9,99€) hai a disposizione 2 potenziamenti che puoi utilizzare in qualunque server tu voglia. Se deciderai di potenziare noi, __Vinili & Caffè__, sbloccherai un sacco di vantaggi.","**Non sai cos'è Discord Nitro?** <:link:1470064815899803668> [Scoprilo qui](<https://discord.com/nitro>).","",`<:sparkledred:1470064814502973591>・Ruolo <@&${IDs.roles.ServerBooster}>con badge speciale`,
            "<:moon:1470064812615667827>・Permesso di allegare immagini e link in chat",
      ...
      const howtoEmbed=new EmbedBuilder().setImage(DIVIDER_URL).setColor("#6f4e37").setTitle("<:nitroboost:1470064881674883326>・**__Come creare ruolo personalizzato e vocale privata__**",).setDescription([`Usa\`+customrole create\` in <#${IDs.channels.commands}>per creare e configurare il ruolo.`,
    ...
    const buildLevelsPayload=() => {const levelEmbed=new EmbedBuilder().setImage(DIVIDER_URL).setColor("#6f4e37").setTitle("<:exp:1470067108543987846>・**__Sali di livello e sblocca vantaggi sempre migliori__**",).setDescription(["I livelli nel server rappresentano la tua attività: scrivendo in chat testuale e stando nei canali vocali, guadagnerai esperienza che verrà aggiunta al tuo livello globale. ","Una volta raggiunta una certa somma di esperienza, farai un **level up**!","",`<:dot:1443660294596329582> Per __vedere i tuoi exp__ e le tue statistiche, usa i comandi: \`+rank\` in  <#${IDs.channels.commands}>.`,
            "",
