  ...
  async execute(client) {
    ...
    const mentionsMenu = new ActionRowBuilder().addComponents(
      new StringSelectMenuBuilder()
        ...
        .addOptions(
          ...
          { label: 'Revive Chat', value: IDs.roles.supporterLink, emoji: '???', description: 'Clicca qui per ottenere il ruolo' },
          ...
          { label: 'Minigames', value: IDs.roles.verifyExtraB, emoji: '??', description: 'Clicca qui per ottenere il ruolo' },
          { label: 'Forum', value: IDs.roles.forumNotify, emoji: '??', description: 'Clicca qui per ottenere il ruolo' }
        )
    );
    ...
    const plusColorsMenu = new ActionRowBuilder().addComponents(
      new StringSelectMenuBuilder()
        ...
        .setPlaceholder('?? Seleziona un colore il tuo profilo????')
        .setMinValues(1)
        .setMaxValues(1)
        .addOptions(
          { label: 'Rimuovi', value: 'remove', emoji: '?', description: 'Rimuovi ruoli dal tuo profilo' },
