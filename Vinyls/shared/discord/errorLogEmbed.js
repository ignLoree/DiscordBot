...
function buildErrorLogEmbed({ contextLabel, contextValue, userTag, error, title = "<a:VC_Alert:1448670089670037675> Log errori", serverName = null, }) {
  ...
  return new EmbedBuilder()
    ...
    .setTitle(safeTitle)
    .addFields(
      { name: `<:VC_Poll:1448695754972729436> ${safeLabel}`, value: `\`\`\`${safeContext}\`\`\`` },
      { name: "<a:VC_Channel:1448670215444631706> Server", value: `\`\`\`${safeServer}\`\`\`` },
      { name: "<:member_role_icon:1330530086792728618> Utente", value: `\`\`\`${safeUserTag}\`\`\`` },
      { name: "<a:VC_Alert:1448670089670037675> Errore", value: `\`\`\`${errorInBlock}\`\`\`` },
    )
    .setTimestamp();
}
