...
function badgeNames(userFlags) {
  if (!userFlags) return [];
  const map=[[UserFlagsBitField.Flags.Staff,"Discord Staff"],[UserFlagsBitField.Flags.Partner,"Partner"],[UserFlagsBitField.Flags.Hypesquad,"HypeSquad"],[UserFlagsBitField.Flags.BugHunterLevel1,"Bug Hunter 1"],[UserFlagsBitField.Flags.BugHunterLevel2,"Bug Hunter 2"],[UserFlagsBitField.Flags.HypeSquadOnlineHouse1,"House Bravery"],[UserFlagsBitField.Flags.HypeSquadOnlineHouse2,"House Brilliance"],[UserFlagsBitField.Flags.HypeSquadOnlineHouse3,"House Balance"],[UserFlagsBitField.Flags.PremiumEarlySupporter,"Early Supporter"],[UserFlagsBitField.Flags.VerifiedDeveloper,"Verified Developer"],[UserFlagsBitField.Flags.ActiveDeveloper,"Active Developer"],[UserFlagsBitField.Flags.VerifiedBot,"Verified Bot"],];
  return map.filter(([flag]) => userFlags.has(flag)).map(([, label]) => label);
}

function dangerousPerms(member) {
  const checks=[[PermissionsBitField.Flags.KickMembers,"Kick Members"],[PermissionsBitField.Flags.BanMembers,"Ban Members"],[PermissionsBitField.Flags.ManageChannels,"Manage Channels"],[PermissionsBitField.Flags.ManageGuild,"Manage Guild"],[PermissionsBitField.Flags.ManageMessages,"Manage Messages"],[PermissionsBitField.Flags.ManageRoles,"Manage Roles"],[PermissionsBitField.Flags.ModerateMembers,"Moderate Members"],[PermissionsBitField.Flags.Administrator,"Administrator"],];
  return checks
...
function rolesPreview(member) {
  const roles=[...member.roles.cache.values()].filter((r) => r.id!==member.guild.id).sort((a,b) => b.position-a.position);
...
function buildStrikesEmbed(member, strikes, page, totalPages) {
  ...
  const rows=current.length?current.map((item,i) => `\`${safePage*5+i+1}\` ${item}`)
...
module.exports = {
  ...
  async execute(message, args) {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        [
          ...
          dgPerms.length ? `<:space:1461733157840621608><:rightDoubleArrow:1465450678062288937>${dgPerms.join("  <:VC_right_arrow:1482459908245815296>  ")}` : "<:space:1461733157840621608><:rightDoubleArrow:1465450678062288937> none",
          "",
          "**Bot Permissions:**",
          `🤖 This user has Bot permissions ${boolIcon(wkPerms.length > 0)}`,
          wkPerms.length ? `<:space:1461733157840621608><:rightDoubleArrow:1465450678062288937>${wkPerms.join("  <:VC_right_arrow:1482459908245815296>  ")}` : "<:space:1461733157840621608><:rightDoubleArrow:1465450678062288937> none",
          "",
          "**Account Accessories:**",
    ...
    const strikeItems=Array.isArray(auto.strikeReasons)?auto.strikeReasons.map((x) => String(x||"").trim()).filter(Boolean):[];
    ...
    if (totalPages > 1) {
      ...
      let currentPage = 0;
      const collector=sent.createMessageComponentCollector({componentType:ComponentType.Button,time:10*60_000,});
