...
function keepPreviousOrder(items, previousEntries, getName) {
  ...
  for (const prev of previous) {
    ...
    if (prevId) {
      idx = list.findIndex(
        (item, i) => !used.has(i) && String(item?.id || "") === prevId,
      );
    }
    if (idx === -1 && prevName) {
      idx = list.findIndex(
        (item, i) => !used.has(i) && String(getName(item)) === prevName,
      );
    }
    if (idx !== -1) {
      used.add(idx);
...
function renderCatalogFile({ categoriesLines, channelsLines, rolesLines, botsLines, emojisLines, guildId, voteLink, inviteLink, loadingAnimatedId, loadingFallbackId }) {
  const categoriesRawBlock = buildRawBlock(categoriesLines);
  const channelsRawBlock = buildRawBlock(channelsLines);
  const rolesRawBlock = buildRawBlock(rolesLines);
  const botsRawBlock = buildRawBlock(botsLines);
  const emojisRawBlock = buildRawBlock(emojisLines);
  const metaGuild = JSON.stringify(guildId || null);
  const metaLoadingAnim = JSON.stringify(loadingAnimatedId || null);
  const metaLoadingFallback = JSON.stringify(loadingFallbackId || null);
  const metaVote = JSON.stringify(voteLink || null);
  const metaInvite = JSON.stringify(inviteLink || null);

  const tail = [
    "const categoriesRaw = " + categoriesRawBlock + ";",
    "const channelsRaw = " + channelsRawBlock + ";",
    "const rolesRaw = " + rolesRawBlock + ";",
    "const botsRaw = " + botsRawBlock + ";",
    "const emojisRaw = " + emojisRawBlock + ";",
    "",
    ...
    "  meta: { guildMain: " + metaGuild + ", emojis: { loadingAnimatedId: " + metaLoadingAnim + ", loadingFallbackId: " + metaLoadingFallback + " }, links: { vote: " + metaVote + ", invite: " + metaInvite + " } }",
    "};",
  ].join("\n");

  return IDS_CATALOG_HEAD + "\n" + tail;
}
...
async function collectGuildCatalog(guild, ids) {
  ...
  const rawEntries = ids?.namedEntries || {};
  const previousEntries = {
    categories: Array.isArray(rawEntries.categories) ? rawEntries.categories : [],
    channels: Array.isArray(rawEntries.channels) ? rawEntries.channels : [],
    roles: Array.isArray(rawEntries.roles) ? rawEntries.roles : [],
    bots: Array.isArray(rawEntries.bots) ? rawEntries.bots : [],
    emojis: Array.isArray(rawEntries.emojis) ? rawEntries.emojis : [],
  };
  ...
  return { categoriesLines, channelsLines, rolesLines, botsLines, emojisLines, catalogSource };
}
...
module.exports = { collectGuildCatalog, writeCatalogFiles };