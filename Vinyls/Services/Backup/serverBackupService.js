...
async function collectArchivedThreads(parentChannel) {
  ...
  const archivedPublic = await parentChannel.threads.fetchArchived({ type: "public", fetchAll: true }).catch(() => null);
  if (archivedPublic?.threads) {
    threadList.push(...archivedPublic.threads.values());
  }

  const archivedPrivate = await parentChannel.threads.fetchArchived({ type: "private", fetchAll: true }).catch(() => null);
...
function getBackupFilePath(guildId, backupId) {
  const normalizedId = String(backupId || "").trim().toUpperCase();
...
async function createGuildBackup(guild, options = {}) {
  ...
  const [guildIconData, guildBannerData, guildSplashData, guildDiscoverySplashData,] = await Promise.all([fetchAssetAsDataUri(guild.iconURL({ extension: "png", size: 1024 })), fetchAssetAsDataUri(guild.bannerURL({ extension: "png", size: 2048 })), fetchAssetAsDataUri(guild.splashURL?.({ extension: "png", size: 2048 })), fetchAssetAsDataUri(guild.discoverySplashURL?.({ extension: "png", size: 2048 }),),]);

  const outputDir = getBackupFolder(guild.id);
  await fs.mkdir(outputDir, { recursive: true });

  const [membersCollection, bansCollection, invitesCollection, webhooksCollection] = await Promise.all([guild.members.fetch().catch(() => null), guild.bans.fetch().catch(() => null), guild.invites.fetch().catch(() => null), guild.fetchWebhooks().catch(() => null),]);
  ...
  const textLikeChannels = channels.filter((channel) => channel?.isTextBased?.() && typeof channel?.messages?.fetch === "function" && !channel?.isDMBased?.(),);
  ...
  const roles = [...guild.roles.cache.values()].sort((a, b) => b.position - a.position).map((role) => ({ id: String(role.id), name: String(role.name || ""), color: Number(role.color || 0), hoist: Boolean(role.hoist), icon: role.icon || null, iconURL: role.iconURL({ extension: "png", size: 512 }), unicodeEmoji: role.unicodeEmoji || null, position: Number(role.position || 0), permissions: String(role.permissions?.bitfield ?? 0n), managed: Boolean(role.managed), mentionable: Boolean(role.mentionable), tags: role.tags ? { ...role.tags } : null, createdAt: toIso(role.createdAt), }));

  const members = membersCollection ? [...membersCollection.values()].map((member) => ({ id: String(member.id), user: { id: String(member.user?.id || member.id), username: String(member.user?.username || ""), discriminator: String(member.user?.discriminator || "0"), tag: String(member.user?.tag || ""), globalName: member.user?.globalName || null, bot: Boolean(member.user?.bot), system: Boolean(member.user?.system), avatar: member.user?.avatar || null, banner: member.user?.banner || null, accentColor: member.user?.accentColor ?? null, createdAt: toIso(member.user?.createdAt), }, nickname: member.nickname || null, displayName: member.displayName || null, joinedAt: toIso(member.joinedAt), premiumSince: toIso(member.premiumSince), communicationDisabledUntil: toIso(member.communicationDisabledUntil), pending: Boolean(member.pending), roles: member.roles?.cache ? [...member.roles.cache.keys()].map(String) : [], avatar: member.avatar || null, avatarURL: member.displayAvatarURL({ size: 1024 }), })) : [];

  const emojis = [...guild.emojis.cache.values()].map((emoji) => ({ id: String(emoji.id), name: String(emoji.name || ""), animated: Boolean(emoji.animated), managed: Boolean(emoji.managed), available: Boolean(emoji.available), createdAt: toIso(emoji.createdAt), url: emoji.imageURL?.() ?? null, roles: emoji.roles?.cache ? [...emoji.roles.cache.keys()].map(String) : [], userId: emoji.author?.id ? String(emoji.author.id) : null, }));

  const stickers = [...guild.stickers.cache.values()].map((sticker) => ({ id: String(sticker.id), name: String(sticker.name || ""), description: sticker.description || null, format: Number(sticker.format ?? 0), type: Number(sticker.type ?? 0), tags: sticker.tags || null, url: sticker.url || null, available: Boolean(sticker.available), guildId: sticker.guildId ? String(sticker.guildId) : null, userId: sticker.user?.id ? String(sticker.user.id) : null, }));

  const scheduledEvents = [...guild.scheduledEvents.cache.values()].map((ev) => ({ id: String(ev.id), name: String(ev.name || ""), description: ev.description || null, entityType: Number(ev.entityType ?? 0), status: Number(ev.status ?? 0), privacyLevel: Number(ev.privacyLevel ?? 0), entityMetadata: ev.entityMetadata || null, channelId: ev.channelId ? String(ev.channelId) : null, creatorId: ev.creatorId ? String(ev.creatorId) : null, userCount: Number(ev.userCount ?? 0), scheduledStartAt: toIso(ev.scheduledStartTimestamp || ev.scheduledStartAt), scheduledEndAt: toIso(ev.scheduledEndTimestamp || ev.scheduledEndAt), createdAt: toIso(ev.createdAt), image: ev.image || null, }));
  ...
  const backupSource = String(options?.source || "manual").trim().toLowerCase();

  const backupPayload = { schemaVersion: BACKUP_SCHEMA_VERSION, backupId, createdAt: createdAt.toISOString(), source: backupSource === "automatic" ? "automatic" : "manual", guild: { id: String(guild.id), name: String(guild.name || ""), icon: guild.icon || null, iconURL: guild.iconURL({ size: 2048 }), iconData: guildIconData, banner: guild.banner || null, bannerURL: guild.bannerURL({ size: 2048 }), bannerData: guildBannerData, splash: guild.splash || null, splashURL: guild.splashURL?.({ size: 2048 }) || null, splashData: guildSplashData, discoverySplash: guild.discoverySplash || null, discoverySplashURL: guild.discoverySplashURL?.({ size: 2048 }) || null, discoverySplashData: guildDiscoverySplashData, description: guild.description || null, vanityURLCode: guild.vanityURLCode || null, preferredLocale: guild.preferredLocale || null, features: Array.isArray(guild.features) ? guild.features : [], verificationLevel: Number(guild.verificationLevel ?? 0), explicitContentFilter: Number(guild.explicitContentFilter ?? 0), defaultMessageNotifications: Number(guild.defaultMessageNotifications ?? 0), mfaLevel: Number(guild.mfaLevel ?? 0), nsfwLevel: Number(guild.nsfwLevel ?? 0), premiumTier: Number(guild.premiumTier ?? 0), premiumProgressBarEnabled: Boolean(guild.premiumProgressBarEnabled), afkTimeout: Number(guild.afkTimeout ?? 0), afkChannelId: guild.afkChannelId ? String(guild.afkChannelId) : null, systemChannelId: guild.systemChannelId ? String(guild.systemChannelId) : null, rulesChannelId: guild.rulesChannelId ? String(guild.rulesChannelId) : null, publicUpdatesChannelId: guild.publicUpdatesChannelId ? String(guild.publicUpdatesChannelId) : null, safetyAlertsChannelId: guild.safetyAlertsChannelId ? String(guild.safetyAlertsChannelId) : null, widgetEnabled: Boolean(guild.widgetEnabled), widgetChannelId: guild.widgetChannelId ? String(guild.widgetChannelId) : null, ownerId: guild.ownerId ? String(guild.ownerId) : null, maxMembers: Number(guild.maximumMembers ?? 0), maxPresences: Number(guild.maximumPresences ?? 0), maxVideoChannelUsers: Number(guild.maxVideoChannelUsers ?? 0), memberCount: Number(guild.memberCount ?? 0), rolesCount: Number(guild.roles.cache.size || 0), channelsCount: Number(channels.length), threadsCount: Number(serializedThreads.length), emojisCount: Number(guild.emojis.cache.size || 0), stickersCount: Number(guild.stickers.cache.size || 0), createdAt: toIso(guild.createdAt), }, roles, channels: serializedChannels, threads: serializedThreads, members, bans: bansCollection ? [...bansCollection.values()].map((ban) => ({ user: { id: String(ban.user.id), username: String(ban.user.username || ""), discriminator: String(ban.user.discriminator || "0"), tag: String(ban.user.tag || ""), bot: Boolean(ban.user.bot), createdAt: toIso(ban.user.createdAt), }, reason: ban.reason || null, })) : [], invites: invitesCollection ? [...invitesCollection.values()].map((invite) => ({ code: String(invite.code), url: invite.url || null, channelId: invite.channelId ? String(invite.channelId) : null, inviterId: invite.inviterId ? String(invite.inviterId) : null, targetType: invite.targetType ?? null, targetUserId: invite.targetUserId ? String(invite.targetUserId) : null, expiresAt: toIso(invite.expiresAt), createdAt: toIso(invite.createdAt), temporary: Boolean(invite.temporary), maxAge: Number(invite.maxAge ?? 0), maxUses: Number(invite.maxUses ?? 0), uses: Number(invite.uses ?? 0), })) : [], webhooks: webhooksCollection ? [...webhooksCollection.values()].map((hook) => ({ id: String(hook.id), type: Number(hook.type ?? 0), guildId: hook.guildId ? String(hook.guildId) : null, channelId: hook.channelId ? String(hook.channelId) : null, name: hook.name || null, avatar: hook.avatar || null, applicationId: hook.applicationId ? String(hook.applicationId) : null, sourceGuildId: hook.sourceGuild?.id ? String(hook.sourceGuild.id) : null, sourceChannelId: hook.sourceChannel?.id ? String(hook.sourceChannel.id) : null, })) : [], emojis, stickers, scheduledEvents, integrations: Array.isArray(integrations) ? integrations.map((integration) => ({ id: String(integration.id), name: String(integration.name || ""), type: String(integration.type || ""), enabled: Boolean(integration.enabled), syncing: Boolean(integration.syncing), roleId: integration.roleId ? String(integration.roleId) : null, expireBehavior: integration.expireBehavior ?? null, expireGracePeriod: integration.expireGracePeriod ?? null, userId: integration.user?.id ? String(integration.user.id) : null, account: integration.account ? { id: String(integration.account.id || ""), name: String(integration.account.name || ""), } : null, syncedAt: toIso(integration.syncedAt), subscriberCount: Number(integration.subscriberCount ?? 0), revoked: Boolean(integration.revoked), application: integration.application ? { id: String(integration.application.id || ""), name: String(integration.application.name || ""), icon: integration.application.icon || null, } : null, })) : [], autoModerationRules: autoModerationRules ? [...autoModerationRules.values()].map((rule) => ({ id: String(rule.id), name: String(rule.name || ""), creatorId: rule.creatorId ? String(rule.creatorId) : null, eventType: Number(rule.eventType ?? 0), triggerType: Number(rule.triggerType ?? 0), triggerMetadata: typeof rule.triggerMetadata?.toJSON === "function" ? rule.triggerMetadata.toJSON() : rule.triggerMetadata || null, enabled: Boolean(rule.enabled), exemptChannels: Array.isArray(rule.exemptChannels) ? rule.exemptChannels.map(String) : [], exemptRoles: Array.isArray(rule.exemptRoles) ? rule.exemptRoles.map(String) : [], actions: Array.isArray(rule.actions) ? rule.actions.map((action) => typeof action?.toJSON === "function" ? action.toJSON() : action,) : [], })) : [], auditLogs, messages: { channels: messagesByChannel, threads: messagesByThread, }, };

  const serialized = JSON.stringify(backupPayload);
  const compressed = await gzipAsync(serialized, { level: zlib.constants.Z_BEST_COMPRESSION, });
...
async function readGuildBackup(guildId, backupId) {
  ...
  const tryRead = async (targetPath, sourceLabel) => { const compressed = await fs.readFile(targetPath); const compressedSha256 = sha256(compressed); const uncompressed = await gunzipAsync(compressed); const serialized = uncompressed.toString("utf8"); const payloadSha256 = sha256(Buffer.from(serialized, "utf8")); if (expectedCompressed && expectedCompressed !== compressedSha256.toLowerCase()) { const err = new Error("checksum compressed mismatch"); err.code = "EBADCHK"; throw err; } if (expectedPayload && expectedPayload !== payloadSha256.toLowerCase()) { const err = new Error("checksum payload mismatch"); err.code = "EBADCHK"; throw err; } const payload = JSON.parse(serialized); return { filePath: targetPath, sizeBytes: compressed.length, payload, meta, checksum: { algorithm: "sha256", compressed: compressedSha256, payload: payloadSha256, compressedVerified: expectedCompressed ? true : null, payloadVerified: expectedPayload ? true : null, }, source: sourceLabel, }; };
...
async function listGuildBackupMetas(guildId, { search = "", limit = 25, offset = 0 } = {}) {
  ...
  const backupFiles = files.filter((name) => String(name).toLowerCase().endsWith(".json.gz")).slice(0, 250);

  const withStats = await Promise.all(backupFiles.map(async (name) => { const filePath = path.join(folder, name); const stat = await fs.stat(filePath).catch(() => null); return { name, mtimeMs: Number(stat?.mtimeMs || 0), }; }),);
...
async function listGuildBackupMetasPaginated(guildId, { search = "", page = 1, pageSize = 10 } = {}) {
  ...
  const all = await listGuildBackupMetas(guildId, { search, limit: 500, offset: 0, });
...
async function pruneGuildBackups(guildId, { maxManual = 20, maxAutomatic = 1, maxManualAgeDays = 30, minManualToKeep = 5 } = {}) {
  ...
  const manualDeleteByAge = manualSorted.filter((entry) => { if (manualKeepSet.has(String(entry.backupId))) return false; const createdMs = toMs(entry?.createdAt); return createdMs > 0 && createdMs < ageCutoff; });

  const manualAfterAge = manualSorted.filter((entry) => !manualDeleteByAge.some((d) => d.backupId === entry.backupId),);
  ...
  const toDelete = [...manualDeleteByAge, ...manualDeleteByCount, ...automaticToDelete,];
...
async function validateAndHealGuildBackups(guildId, { limit = 30 } = {}) {
  const metas = await listGuildBackupMetas(guildId, { limit: Math.max(1, Math.min(200, Number(limit || 30))), });
  const healed = [];
  const failed = [];
...
async function writeSecuritySnapshot(guildId, payload, options = {}) {
  ...
  const entry = { id: snapshotId, createdAt, guildId: String(guildId || ""), actorId: String(options.actorId || ""), reason: String(options.reason || "manual"), payload: payload && typeof payload === "object" ? payload : {}, };
...
async function listSecuritySnapshots(guildId, limit = 30) {
  ...
  const files = await fs.readdir(folder).catch(() => []);
  const secFiles = files.filter((name) => name.startsWith(SECURITY_SNAPSHOT_PREFIX) && name.toLowerCase().endsWith(".json.gz")).map((name) => name.slice(0, -8));
  const withMeta = await Promise.all(secFiles.map(async (snapshotId) => { const meta = await readBackupMetaFile(effectiveGuildId, snapshotId).catch(() => null); return { id: snapshotId, createdAt: meta?.createdAt ? new Date(meta.createdAt).getTime() : 0, guildId: String(meta?.guildId ?? effectiveGuildId), actorId: String(meta?.actorId ?? ""), reason: String(meta?.reason ?? ""), }; }),);
...
async function readSecuritySnapshot(guildId, snapshotId) {
  ...
  const tryRead = async (targetPath) => { const compressed = await fs.readFile(targetPath); const uncompressed = await gunzipAsync(compressed); const serialized = uncompressed.toString("utf8"); const entry = JSON.parse(serialized); return entry; };
  try {
