...
const { isSecurityProfileImmune, hasAdminsProfileCapability, getSecurityStaticsSnapshot, } = require("./securityProfilesService");
...
const DANGEROUS_PERMS = [PermissionsBitField.Flags.Administrator, PermissionsBitField.Flags.ManageGuild, PermissionsBitField.Flags.ManageRoles, PermissionsBitField.Flags.ManageChannels, PermissionsBitField.Flags.ManageWebhooks, PermissionsBitField.Flags.BanMembers, PermissionsBitField.Flags.KickMembers,];
const DANGEROUS_CHANNEL_PERMS = [PermissionsBitField.Flags.ManageChannels, PermissionsBitField.Flags.ManageRoles, PermissionsBitField.Flags.ManageWebhooks, PermissionsBitField.Flags.ManageMessages, PermissionsBitField.Flags.MentionEveryone,];
const LOCKDOWN_CHANNEL_PERMISSION_FLAGS = [["SendMessages", PermissionsBitField.Flags.SendMessages], ["AddReactions", PermissionsBitField.Flags.AddReactions], ["CreatePublicThreads", PermissionsBitField.Flags.CreatePublicThreads], ["CreatePrivateThreads", PermissionsBitField.Flags.CreatePrivateThreads], ["SendMessagesInThreads", PermissionsBitField.Flags.SendMessagesInThreads], ["Connect", PermissionsBitField.Flags.Connect], ["Speak", PermissionsBitField.Flags.Speak], ["Stream", PermissionsBitField.Flags.Stream], ["UseApplicationCommands", PermissionsBitField.Flags.UseApplicationCommands],].filter(([, bit]) => Boolean(bit));
const LOCKDOWN_EXTRA_ROLE_IDS = [IDs.roles?.Member, IDs.roles?.Staff,].map((id) => String(id || "").trim()).filter(Boolean);
...
const QUARANTINE_ROLE_SNAPSHOTS = new Map();
const ANTINUKE_CONFIG_PATH = path.resolve(__dirname, "../../Utils/Config/antiNukeConfig.json",);
const ANTINUKE_CONFIG = { enabled: true, detectPrune: false, vanityGuard: true, autoQuarantine: { enabled: true, strictMode: true, strictMemberRoleAddition: true, monitorPublicRoles: true, monitorChannelPermissions: true, quarantineRoleId: "1442568884833095832", quarantineTimeoutMs: 24 * 60 * 60_000, whitelistUserIds: new Set(["610531025470095390", "700814270069342339", "1466495522474037463", "1329118940110127204",]), }, kickBanFilter: { enabled: true, minuteLimit: 5, hourLimit: 15, heatPerAction: 20, }, roleCreationFilter: { enabled: true, minuteLimit: 5, hourLimit: 15, heatPerAction: 10, }, roleDeletionFilter: { enabled: true, minuteLimit: 3, hourLimit: 10, heatPerAction: 25, }, channelCreationFilter: { enabled: true, minuteLimit: 4, hourLimit: 12, heatPerAction: 16, }, channelDeletionFilter: { enabled: true, minuteLimit: 3, hourLimit: 8, heatPerAction: 25, }, webhookCreationFilter: { enabled: true, minuteLimit: 3, hourLimit: 10, heatPerAction: 15, }, webhookUpdateFilter: { enabled: true, minuteLimit: 4, hourLimit: 12, heatPerAction: 12, }, webhookDeletionFilter: { enabled: true, minuteLimit: 3, hourLimit: 8, heatPerAction: 10, }, inviteCreationFilter: { enabled: true, minuteLimit: 4, hourLimit: 15, heatPerAction: 12, }, panicMode: { enabled: true, useHeatAlgorithm: true, thresholdHeat: 130, decayPerSec: 5, durationMs: 10 * 60_000, maxDurationMs: 45 * 60_000, extendByMsOnTrigger: 2 * 60_000, lockdown: { dangerousRoles: true, unlockDangerousRolesOnFinish: true, lockModerationCommands: true, lockAllCommands: true, channelLockdown: true, banAttackExecutor: true, roleAllowlistIds: new Set([]), }, warnedRoleIds: new Set([String(IDs.roles.HighStaff || ""),].filter(Boolean)), whitelistCategoryIds: new Set([]), autoBackupSync: { enabled: false, restoreDeletedRoles: true, deleteNewRoles: true, restoreDeletedChannels: true, deleteNewChannels: true, deleteNewWebhooks: true, }, instantRollbackWhileActive: { enabled: true, quarantineExecutor: true, deleteCreatedRoles: true, deleteCreatedChannels: true, deleteCreatedWebhooks: true, }, }, };
...
function applyPersistentAntiNukeConfig(raw) {
  ...
  const clamp = (value, min, max, fallback) => { const n = Number(value); if (!Number.isFinite(n)) return fallback; return Math.min(max, Math.max(min, n)); };
  ...
  const filters = [ANTINUKE_CONFIG.kickBanFilter, ANTINUKE_CONFIG.roleCreationFilter, ANTINUKE_CONFIG.roleDeletionFilter, ANTINUKE_CONFIG.channelCreationFilter, ANTINUKE_CONFIG.channelDeletionFilter, ANTINUKE_CONFIG.webhookCreationFilter, ANTINUKE_CONFIG.webhookUpdateFilter, ANTINUKE_CONFIG.webhookDeletionFilter, ANTINUKE_CONFIG.inviteCreationFilter,];
...
function formatExecutorLine(executorId) {
  ...
  if (!actorId || actorId === UNKNOWN_EXECUTOR_ID) {
    return `<:VC_Mention:1482526855289634997> **Executor:** Sconosciuto`;
  }
  return `<:VC_Mention:1482526855289634997> **Executor:** <@${actorId}> \`${actorId}\``;
}

function formatAuditStatusLine(executorId) {
  if (isUnknownExecutorId(executorId)) {
    return `<:VC_reason:1478517122929004544> **Audit Status:** Missing audit entry from Discord`;
  }
  return `<:VC_reason:1478517122929004544> **Audit Status:** Audit entry resolved`;
}
...
async function logAuditMissingSkip(guild, title, lines = []) {
  await sendAntiNukeLog(
    ...
    [
      ...
      ...lines,
      `<:VC_reason:1478517122929004544> **Technical Reason:** Audit log entry not available in time, automatic enforcement skipped`,
      `<:VC_reason:1478517122929004544> **Result:** No enforcement performed`,
    ].filter(Boolean),
    "#FEE75C",
  );
}
...
function getMainRoleIds(guild) {
  ...
  const dynamicMainRoles = Array.isArray(statics?.mainRoleIds) ? statics.mainRoleIds.map((id) => String(id || "").trim()).filter(Boolean) : [];
...
function hasStaffProtection(member) {
  ...
  const protectedRoleIds = [IDs.roles.Founder, IDs.roles.CoFounder,].map((id) => String(id || "").trim()).filter(Boolean);
...
function extractLineValue(lines, label) {
  const key = `**${label}:**`;
  const raw = (Array.isArray(lines) ? lines : []).find((line) => String(line || "").includes(key),);
  if (!raw) return "";
  return String(raw)
    .replace(/<:[^>]+>\s*/g, "")
    .replace(key, "")
...
function antiNukeHeadline(title, lines) {
  const executor = extractLineValue(lines, "<:VC_Mention:1482526855289634997> Executor");
  const action = extractLineValue(lines, "<:VC_Mention:1482526855289634997> Action");
  if (/panic mode enabled/i.test(String(title || ""))) {
    return "<:success:1461731530333229226> AntiNuke panic mode has been enabled!";
  }
  if (/panic mode ended/i.test(String(title || ""))) {
    return "<:cancel:1461730653677551691> AntiNuke panic mode has ended.";
  }
  if (executor && /quarantined/i.test((lines || []).join(" "))) {
    return `<:VC_Lock:1482526739044368476> ${executor.split(" ")[0]} has been quarantined!`;
  }
  if (action) return `<a:VC_Alert:1448670089670037675> AntiNuke action: ${action}`;
  return title || "<a:VC_Alert:1448670089670037675> AntiNuke Trigger";
}
...
function cleanAntiNukeLines(lines) {
  return (Array.isArray(lines) ? lines : [])
    ...
    .filter(
      (line) =>
        !line.startsWith("<:VC_Mention:1482526855289634997> **Executor:**") &&
        !line.startsWith("<:VC_Mention:1482526855289634997> **Target:**") &&
        !line.startsWith("<:channeltext:1443247596922470551> **Channel:**"),
    );
}
...
async function sendAntiNukeLog(guild, title, lines, color = "#ED4245") {
  ...
  const dedupeBasis = [String(guild?.id || ""), String(title || ""), String(color || ""), ...(Array.isArray(lines) ? lines.map((x) => String(x || "").trim()) : []),].join("|");
  const now = Date.now();
  ...
  const executor = extractLineValue(lines, "<:VC_Mention:1482526855289634997> Executor");
  const target = extractLineValue(lines, "<:VC_Mention:1482526855289634997> Target");
  const channel = extractLineValue(lines, "<:channeltext:1443247596922470551> Channel");
  const filterName = title?.replace(/^AntiNuke:\s*/i, "") || "<a:VC_Alert:1448670089670037675> AntiNuke";
  ...
  channel
    ...
    : null,
  target
    ? `<:VC_Mention:1482526855289634997> **Target:** ${target}`
    : null,
  executor
    ? `<:VC_Mention:1482526855289634997> **Member:** ${executor}`
    : null,
    "",
  ...details.map(
    (line) => `<:VC_reason:1478517122929004544>${line}`,
  ),
  ]
    .filter(Boolean)
    .join("\n"),
...
function getPanicState(guildId) {
  ...
  for (const [gid, state] of ANTINUKE_PANIC_STATE.entries()) {
    ...
    const hasArtifacts = Number(state?.lockedRoles?.size || 0) > 0 || Number(state?.lockedChannels?.size || 0) > 0 || Number(state?.createdRoleIds?.size || 0) > 0 || Number(state?.createdChannelIds?.size || 0) > 0 || Number(state?.createdWebhookIds?.size || 0) > 0 || Number(state?.deletedRoleSnapshots?.size || 0) > 0 || Number(state?.deletedChannelSnapshots?.size || 0) > 0;
...
async function sendPanicModeDetailedReport(
  ...
  if (backupSummary) {
    ...
    incrementPanicMetric(state, "<:VC_Mention:1482526855289634997> roles", "<:cancel:1461730653677551691> deleted", Number(backupSummary.rolesDeleted || 0));
    incrementPanicMetric(state, "<:VC_Mention:1482526855289634997> roles", "<:success:1461731530333229226> recovered", Number(backupSummary.rolesRecovered || 0));
    incrementPanicMetric(
      state,
      "<:channeltext:1443247596922470551> channels",
      "<:cancel:1461730653677551691> deleted",
      Number(backupSummary.channelsDeleted || 0),
    );
    incrementPanicMetric(
      state,
      "<:channeltext:1443247596922470551> channels",
      "<:success:1461731530333229226> recovered",
  ...
  const backupLabel = backupEnabled ? report.backup.loaded ? "<:success:1461731530333229226> Backup/restore completato." : "<:cancel:1461730653677551691> Backup abilitato ma senza modifiche applicate." : "<:cancel:1461730653677551691> Backup disabilitato.";

  const lines = [
    `<a:VC_Alert:1448670089670037675> **Panic Mode Triggered! [Anti Nuke]**`,
    `<:VC_reason:1478517122929004544> Attivato monitoraggio anomalo e lockdown.`,
    "",
    ...
    "",
    `<:VC_Mention:1482526855289634997> **Roles**`,
  ...
  for (const role of guild.roles.cache.values()) {
    ...
    if (safeBits !== current) {
      await role
        .setPermissions(
          safeBits,
          "<:VC_Lock:1482526739044368476> AntiNuke Panic: lock dangerous role permissions",
        )
        .catch(() => { });
      incrementPanicMetric(state, "<:VC_Mention:1482526855289634997> roles", "<:VC_right_arrow:1482459908245815296> edited", 1);
    }
  }
}
...
async function unlockDangerousRolesAfterPanic(guild, state) {
  ...
  for (const [roleId, oldBitsStr] of state.lockedRoles.entries()) {
    ...
    await role
      .setPermissions(oldBits, "<a:VC_Unlock:1482532082466029608> AntiNuke Panic: restore role permissions")
      .catch(() => { });
  }
  state.lockedRoles.clear();
  return { completed: true };
    ...
    for (const targetId of targetIds) {
      ...
      await channel.permissionOverwrites
        .edit(
          ...
          { reason: "<:VC_Lock:1482526739044368476> AntiNuke Panic: global channel lockdown" },
        )
        .catch(() => { });
      ops += 1;
      if (ops % 8 === 0) {
...
async function unlockGuildChannelsAfterPanic(guild, state) {
  ...
  for (const [channelId, snapshot] of state.lockedChannels.entries()) {
    const channel = await getGuildChannelCached(guild, String(channelId), { ttlMs: 30_000, });
    if (!channel?.permissionOverwrites) continue;
    const targetsSnapshot = snapshot?.targets && typeof snapshot.targets === "object" ? snapshot.targets : { [String(guild.id || "")]: snapshot, };
    const restoreTargetIds = Array.from(new Set(Object.keys(targetsSnapshot || {}).concat(targetIds).filter(Boolean),),);
    for (const targetId of restoreTargetIds) {
      ...
      if (!targetSnapshot?.hadOverwrite) {
        await channel.permissionOverwrites
          .delete(targetId, "<a:VC_Unlock:1482532082466029608> AntiNuke Panic: restore channel overwrite")
          .catch(() => { });
        ops += 1;
        if (ops % 8 === 0) {
          await sleep(125);
      ...
      await channel.permissionOverwrites
        .edit(
          ...
          { reason: "<a:VC_Unlock:1482532082466029608> AntiNuke Panic: restore channel permissions" },
        )
        .catch(() => { });
      ops += 1;
      if (ops % 8 === 0) {
...
function schedulePanicRestoreRetry(guild, state, reason = "retry") {
  ...
  state.restoreRetryTimer = setTimeout(async () => {
    ...
    const done = Boolean(roleResult?.completed) && Boolean(channelResult?.completed) && Number(state.lockedRoles?.size || 0) === 0 && Number(state.lockedChannels?.size || 0) === 0;
    if (done) {
    ...
    await sendAntiNukeLog(
      guild,
      "<a:VC_Alert:1448670089670037675> AntiNuke Panic Restore Pending",
      [
        `<:VC_reason:1478517122929004544> **Reason:** restore retry limit reached`,
        `<:VC_reason:1478517122929004544> **Locked Roles Pending:** ${Number(state.lockedRoles?.size || 0)}`,
        `<:VC_reason:1478517122929004544> **Locked Channels Pending:** ${Number(state.lockedChannels?.size || 0)}`,
      ],
      "#F59E0B",
    );
  }, delayMs);
...
function buildChannelBackupSnapshot(channel) {
  ...
  const overwrites = Array.from(channel.permissionOverwrites?.cache?.values?.() || []).map((ov) => ({ id: String(ov?.id || ""), type: Number(ov?.type ?? 0), allow: String(BigInt(ov?.allow?.bitfield || 0n)), deny: String(BigInt(ov?.deny?.bitfield || 0n)), })).filter((ov) => ov.id);
  return {
...
async function restoreDeletedRoleSnapshots(guild, state) {
  ...
  for (const snap of pending) {
    const payload = { name: String(snap.name || "restored-role"), color: Number(snap.color || 0), hoist: Boolean(snap.hoist), mentionable: Boolean(snap.mentionable), permissions: BigInt(String(snap.permissions || "0")), reason: "AntiNuke restore: deleted role snapshot", };
    ...
    await role.setPosition(targetPosition, { reason: "<a:VC_Unlock:1482532082466029608> AntiNuke restore: role position" }).catch(() => { });
    restored += 1;
  }
...
async function restoreDeletedChannelSnapshots(guild, state) {
  ...
  for (const snap of pending) {
    ...
    const parentChannel = parentId ? await getGuildChannelCached(guild, parentId, { ttlMs: 30_000 }) : null;
    const overwritePayload = (Array.isArray(snap.permissionOverwrites) ? snap.permissionOverwrites : []).map((ov) => ({ id: String(ov?.id || ""), type: Number(ov?.type ?? 0), allow: BigInt(String(ov?.allow || "0")), deny: BigInt(String(ov?.deny || "0")), })).filter((ov) => { if (!ov.id) return false; if (ov.type === OverwriteType.Role) { return ov.id === String(guild.id || "") || guild.roles.cache.has(ov.id); } if (ov.type === OverwriteType.Member) { return guild.members.cache.has(ov.id); } return false; }).filter((ov) => ov.id);

    const createPayload = { name: String(snap.name || "restored-channel"), type, parent: parentChannel?.type === 4 ? String(parentChannel.id) : null, permissionOverwrites: overwritePayload, reason: "AntiNuke restore: deleted channel snapshot", };
    ...
    if (Number.isFinite(position) && position >= 0) {
      await restoredChannel.setPosition(position, { reason: "<a:VC_Unlock:1482532082466029608> AntiNuke restore: channel position" }).catch(() => { });
    }
    restored += 1;
  }
...
async function runAutoBackupSyncAfterPanic(guild, state) {
  ...
  if (enabled && cfg.deleteNewChannels && me?.permissions?.has(PermissionsBitField.Flags.ManageChannels)) {
    for (const channelId of state.createdChannelIds) {
      const channel = await getGuildChannelCached(guild, String(channelId), { ttlMs: 30_000 });
      if (!channel) continue;
      const deleted = await channel.delete("<a:VC_Unlock:1482532082466029608> AntiNuke panic cleanup: delete new channel").then(() => true).catch(() => false);
      if (deleted) {
  ...
  if (enabled && cfg.deleteNewRoles && me?.permissions?.has(PermissionsBitField.Flags.ManageRoles)) {
    for (const roleId of state.createdRoleIds) {
      const role = await getGuildRoleCached(guild, String(roleId), { ttlMs: 30_000 });
      if (!role) continue;
      if (role.managed) continue;
      if (role.position >= me.roles.highest.position) continue;
      const deleted = await role.delete("<a:VC_Unlock:1482532082466029608> AntiNuke panic cleanup: delete new role").then(() => true).catch(() => false);
  ...
  if (enabled && cfg.deleteNewWebhooks && me?.permissions?.has(PermissionsBitField.Flags.ManageWebhooks)) {
    for (const channel of guild.channels.cache.values()) {
      ...
      for (const webhook of webhooks.values()) {
        ...
        const deleted = await webhook.delete("<a:VC_Unlock:1482532082466029608> AntiNuke panic cleanup: delete new webhook").then(() => true).catch(() => false);
  ...
  const pendingCategorySnapshots = Array.from(state?.deletedChannelSnapshots?.values?.() || [],).filter((snap) => Number(snap?.type) === 4).length;
  const restoredRoles = enabled && cfg.restoreDeletedRoles ? await restoreDeletedRoleSnapshots(guild, state) : { restored: 0, failed: 0 };
  const restoredChannels = enabled && cfg.restoreDeletedChannels ? await restoreDeletedChannelSnapshots(guild, state) : { restored: 0, failed: 0 };

  const recoveredCategories = pendingCategorySnapshots;
  const recoveredChannels = Math.max(0, Number(restoredChannels.restored || 0) - recoveredCategories,);
  ...
  if (
    ...
    await sendAntiNukeLog(
      guild,
      "<a:VC_Alert:1448670089670037675> AntiNuke: Restore System",
      [
        `<:VC_reason:1478517122929004544> **Roles ripristinati:** ${Number(restoredRoles.restored || 0)}`,
        `<:VC_reason:1478517122929004544> **Ruoli falliti:** ${Number(restoredRoles.failed || 0)}`,
        `<:VC_reason:1478517122929004544> **Canali ripristinati:** ${Number(restoredChannels.restored || 0)}`,
        `<:VC_reason:1478517122929004544> **Canali falliti:** ${Number(restoredChannels.failed || 0)}`,
        `<:VC_reason:1478517122929004544> Per ripristino completo canali/ruoli usa \`/backup load\`.`,
      ],
...
async function enableAntiNukePanic(guild, reason, addedHeat = 0, options = {}) {
  ...
  const targetUntil = wasActive ? now + Math.max(baseDuration, extendBy) : now + baseDuration;
  ...
  state.unlockTimer = setTimeout(async () => {
    ...
    const restored = Boolean(roleResult?.completed) && Boolean(channelResult?.completed) && Number(current.lockedRoles?.size || 0) === 0 && Number(current.lockedChannels?.size || 0) === 0;
    if (!restored) {
  ...
  if (!wasActive) {
    await sendAntiNukeLog(
      guild,
      "<a:VC_Alert:1448670089670037675> AntiNuke Panic Mode Enabled",
      [
        `<:VC_reason:1478517122929004544> **Reason:** ${reason || "threshold reached"}`,
        `<:VC_reason:1478517122929004544> **Heat Algorithm:** ${ANTINUKE_CONFIG.panicMode.useHeatAlgorithm ? "ON" : "OFF"}`,
        `<:VC_reason:1478517122929004544> **Duration:** ${Math.round(ANTINUKE_CONFIG.panicMode.durationMs / 60_000)} min`,
        `<:VC_reason:1478517122929004544> **Max Duration:** ${Math.round(maxDuration / 60_000)} min`,
        `<:VC_reason:1478517122929004544> **Dangerous Roles Lockdown:** ${ANTINUKE_CONFIG.panicMode.lockdown.dangerousRoles ? "ON" : "OFF"}`,
        `<:VC_reason:1478517122929004544> **Channel Lockdown:** ${ANTINUKE_CONFIG.panicMode.lockdown.channelLockdown ? "ON" : "OFF"}`,
      ],
      "#ED4245",
    );
  }
...
function scheduleQuarantineRoleRollback(guild, userId, roleId, durationMs) {
  ...
  const timer = setTimeout(async () => {
    ...
    try {
      const member = await getGuildMemberCached(guild, String(userId), { ttlMs: 30_000 });
      const snapshot = QUARANTINE_ROLE_SNAPSHOTS.get(key) || [];
      QUARANTINE_ROLE_SNAPSHOTS.delete(key);
      if (!member) return;
      if (!member.roles.cache.has(String(roleId))) return;
      const me = guild?.members?.me || null;
      const highestPosition = Number(me?.roles?.highest?.position || 0);
      const restoreRoleIds = snapshot
        .map((id) => String(id || "").trim())
        .filter(Boolean)
        .filter((id) => id !== String(guild.id) && id !== String(roleId))
        .filter((id) => {
          const role = guild.roles.cache.get(id);
          if (!role) return false;
          if (role.managed) return false;
          return Number(role.position || 0) < highestPosition;
        });
...
function quarantineOutcomeLabel(outcome) {
  ...
  if (outcome.method === "ban") return `<:VC_Lock:1482526739044368476> Banned ${suspiciousSuffix}`;
  if (outcome.method === "already_role") {
    return `<:VC_Lock:1482526739044368476> Quarantine active ${suspiciousSuffix}`;
  }
  if (outcome.method === "role") return `<:VC_Lock:1482526739044368476> Quarantined via role ${suspiciousSuffix}`;
  if (outcome.method === "timeout") return `<:VC_Lock:1482526739044368476> Quarantined via timeout ${suspiciousSuffix}`;
  return `<:VC_Lock:1482526739044368476> Quarantined${suspiciousSuffix}`;
}
...
async function quarantineExecutor(guild, executorId, reason) {
  ...
  const joinGateSuspicious = await isJoinGateSuspiciousAccount(guild?.id, userId).catch(() => false,);
  ...
  if (
    ...
    const banReason = `<:VC_Lock:1482526739044368476> AntiNuke panic: ${String(reason || "malicious executor")}`;
    const banned = await guild.members.ban(userId, { deleteMessageSeconds: 604800, reason: banReason, }).then(() => true).catch(() => false);
    if (banned) {
      ensurePanicReport(getPanicState(guild?.id)).quarantinedUserIds.add(String(userId));
      if (guild?.client) {
        try {
          const config = await getModConfig(guild.id);
          const { doc, created } = await createModCase({ guildId: guild.id, action: "BAN", userId, modId: guild.client.user.id, reason: banReason, durationMs: null, context: {}, dedupe: { enabled: true, windowMs: 15_000, matchReason: true }, });
          if (created) {
  ...
  const me = guild?.members?.me || null;
  const quarantineRoleId = String(ANTINUKE_CONFIG.autoQuarantine.quarantineRoleId || "",);

  if (
    quarantineRoleId &&
    me?.permissions?.has(PermissionsBitField.Flags.ManageRoles)
  ) {
    const role = await getGuildRoleCached(guild, quarantineRoleId, { ttlMs: 30_000 });
    if (role && role.position < me.roles.highest.position) {
      if (member.roles.cache.has(role.id)) {
        return { applied: true, method: "already_role", joinGateSuspicious };
      }
      const key = `${String(guild?.id || "")}:${String(userId || "")}:${String(role.id || "")}`;
      const snapshotRoleIds = member.roles.cache
        .filter((memberRole) => {
          if (!memberRole?.id) return false;
          if (String(memberRole.id) === String(guild.id)) return false;
          if (String(memberRole.id) === String(role.id)) return false;
          if (memberRole.managed) return false;
          return Number(memberRole.position || 0) < Number(me.roles.highest.position || 0);
        })
        .map((memberRole) => String(memberRole.id));
      QUARANTINE_ROLE_SNAPSHOTS.set(key, snapshotRoleIds);
      const roleApplied = await member.roles.set([String(role.id)], reason).then(() => true).catch(() => false);
      if (!roleApplied) {
        QUARANTINE_ROLE_SNAPSHOTS.delete(key);
      }
      if (roleApplied) {
        const refreshedMember = await getGuildMemberCached(guild, String(userId), { ttlMs: 1 });
        const roleIds = refreshedMember?.roles?.cache ? Array.from(refreshedMember.roles.cache.keys()) : [];
        if (!roleIds.includes(String(role.id)) || roleIds.some((id) => String(id) !== String(guild.id) && String(id) !== String(role.id))) {
          QUARANTINE_ROLE_SNAPSHOTS.delete(key);
  ...
  const timeoutApplied = await member.timeout(quarantineTimeoutMs, reason).then(() => true).catch(() => false);
  if (timeoutApplied) {
    ensurePanicReport(getPanicState(guild?.id)).quarantinedUserIds.add(String(userId));
    if (guild?.client && quarantineTimeoutMs > 0) {
      try {
        const config = await getModConfig(guild.id);
        const { doc, created } = await createModCase({ guildId: guild.id, action: "MUTE", userId, modId: guild.client.user.id, reason: String(reason || "AntiNuke quarantine"), durationMs: quarantineTimeoutMs, context: {}, dedupe: { enabled: true, windowMs: 15_000, matchReason: true }, });
        if (created) {
          await logModCase({ client: guild.client, guild, modCase: doc, config });
        }
      } catch (e) {
...
async function deleteWebhookById(guild, webhookId, preferredChannelId = "") {
  ...
  if (preferredId) {
    const preferredChannel = await getGuildChannelCached(guild, preferredId, { ttlMs: 30_000, });
    if (preferredChannel?.isTextBased?.()) {
      const wh = await preferredChannel.fetchWebhooks().catch(() => null);
      const target = wh?.get?.(id) || null;
      if (target) {
        const deleted = await target.delete("<:VC_Lock:1482526739044368476> AntiNuke panic: delete webhook created during active panic").then(() => true).catch(() => false);
...
function shouldSkipDuplicatedActionEvent({ guildId, executorId, actionKey, targetId = "", now = Date.now() }) {
  cleanupActionDedupe(now);
  const normalizedTarget = String(targetId || "").trim() || "__no_target__";
  const dedupeKey = [String(guildId || ""), String(executorId || ""), String(actionKey || ""), normalizedTarget,].join(":");
  const lastTs = Number(ACTION_EVENT_DEDUPE.get(dedupeKey) || 0);
  ACTION_EVENT_DEDUPE.set(dedupeKey, now);
  const ttlMs = normalizedTarget === "__no_target__" ? ACTION_EVENT_DEDUPE_NO_TARGET_TTL_MS : ACTION_EVENT_DEDUPE_TTL_MS;
  return now - lastTs <= ttlMs;
}

function getTrackerExecutorId(executorId, targetId = "", actionKey = "", now = Date.now()) {
...
function registerGuildBurstActivity(guildId, addedHeat, now = Date.now()) {
  for (const [gid, state] of GUILD_BURST_TRACKER.entries()) {
    ...
    const freshHits = hits.filter((x) => now - Number(x?.ts || 0) <= GUILD_BURST_WINDOW_MS,);
    const recentlyTriggered = now - Number(state?.lastTriggeredAt || 0) <= GUILD_BURST_COOLDOWN_MS;
  ...
  const existing = GUILD_BURST_TRACKER.get(key) || { hits: [], lastTriggeredAt: 0, };
  ...
  const totalHeat = existing.hits.reduce((sum, x) => sum + Number(x?.heat || 0), 0,);
  const canTrigger = totalHeat >= GUILD_BURST_TRIGGER_HEAT && now - Number(existing.lastTriggeredAt || 0) > GUILD_BURST_COOLDOWN_MS;
...
async function applyBurstPanicGuard(guild, actorId, actionLabel, addedHeatPercent) {
  ...
  await sendAntiNukeLog(
    guild,
    "<a:VC_Alert:1448670089670037675> AntiNuke: Coordinated Burst Guard",
    [
      formatExecutorLine(actorId),
      `<a:VC_Alert:1448670089670037675> **Action:** ${actionLabel || "unknown"}`,
      `<:VC_reason:1478517122929004544> **Burst Heat (20s):** ${Math.round(burst.totalHeat)}/${GUILD_BURST_TRIGGER_HEAT}`,
      `<:VC_reason:1478517122929004544> **Result:** Panic mode forced`,
    ].filter(Boolean),
    "#ED4245",
  );
}
...
async function handleKickBanAction({ guild, executorId, action = "unknown", targetId = "" }) {
  ...
  const quarantine = await quarantineExecutor(guild, actorId, "<:VC_Lock:1482526739044368476> AntiNuke: kick/ban abuse detected",);
  await sendAntiNukeLog(
    guild,
    "<a:VC_Alert:1448670089670037675> AntiNuke: Kick/Ban Filter",
    [
      formatExecutorLine(actorId),
      `<a:VC_Alert:1448670089670037675> **Action:** ${action}`,
      targetId
        ? `<a:VC_Alert:1448670089670037675> **Target:** <@${targetId}> \`${targetId}\``
        : null,
      `<a:VC_Alert:1448670089670037675> **Minute Count:** ${minuteCount}/${ANTINUKE_CONFIG.kickBanFilter.minuteLimit}`,
      `<a:VC_Alert:1448670089670037675> **Hour Count:** ${hourCount}/${ANTINUKE_CONFIG.kickBanFilter.hourLimit}`,
      `<a:VC_Alert:1448670089670037675> **Heat:** ${state.heat}/100`,
      `<a:VC_Alert:1448670089670037675> **Result:** ${quarantineOutcomeLabel(quarantine)}`,
    ].filter(Boolean),
  );
}
...
async function handleRoleCreationAction({ guild, executorId, roleId = "" }) {
  ...
  await enableAntiNukePanic(
    guild,
    "<a:VC_Alert:1448670089670037675> Role creation filter",
  ...
  if (panicActive && instantCfg?.enabled && instantCfg?.deleteCreatedRoles && roleId) {
    ...
    if (
      ...
      const removed = await role.delete("<:VC_Lock:1482526739044368476> AntiNuke panic: delete role created during active panic").then(() => true).catch(() => false);
      const canQuarantine = instantCfg?.quarantineExecutor && !shouldSkipPanicExecutorAction(guild.id, actorId, now);
      const quarantine = canQuarantine ? await quarantineExecutor(guild, actorId, "<:VC_Lock:1482526739044368476> AntiNuke panic: role creation blocked during active panic",) : { applied: false, method: "cooldown" };
      if (removed) {
        await sendAntiNukeLog(
          guild,
          "<a:VC_Alert:1448670089670037675> AntiNuke: Panic Instant Rollback",
          [
            formatExecutorLine(actorId),
            `<a:VC_Alert:1448670089670037675> **Action:** Role creation blocked during panic`,
            `<a:VC_Alert:1448670089670037675> **Role:** \`${roleId}\``,
            `<a:VC_Alert:1448670089670037675> **Result:** Role deleted immediately`,
            `<a:VC_Alert:1448670089670037675> **Quarantena executor:** ${quarantineOutcomeLabel(quarantine)}`,
          ],
        );
      }
      return;
  ...
  const minuteCount = state.minuteHits.length;
  const hourCount = state.hourHits.length;
  const exceededMinute = minuteCount >= Number(ANTINUKE_CONFIG.roleCreationFilter.minuteLimit || 5);
  const exceededHour = hourCount >= Number(ANTINUKE_CONFIG.roleCreationFilter.hourLimit || 15);
  const exceededHeat = state.heat >= 100;
  if (!exceededMinute && !exceededHour && !exceededHeat) return;
  ...
  const quarantine = await quarantineExecutor(guild, actorId, "<:VC_Lock:1482526739044368476> AntiNuke: role creation spam detected",);
  await sendAntiNukeLog(
    guild,
    "<a:VC_Alert:1448670089670037675> AntiNuke: Role Creations Filter",
    [
      formatExecutorLine(actorId),
      roleId
        ? `<a:VC_Alert:1448670089670037675> **Last Role:** <@&${roleId}> \`${roleId}\``
        : null,
      `<:VC_reason:1478517122929004544> **Minute Count:** ${minuteCount}/${ANTINUKE_CONFIG.roleCreationFilter.minuteLimit}`,
      `<:VC_reason:1478517122929004544> **Hour Count:** ${hourCount}/${ANTINUKE_CONFIG.roleCreationFilter.hourLimit}`,
      `<:VC_reason:1478517122929004544> **Heat:** ${state.heat}/100`,
      `<:VC_reason:1478517122929004544> **Result:** ${quarantineOutcomeLabel(quarantine)}`,
    ].filter(Boolean),
  );
}
...
async function handleRoleDeletionAction({ guild, executorId, roleName = "", roleId = "", roleSnapshot = null }) {
  ...
  await applyBurstPanicGuard(
    guild,
    actorId,
    "<a:VC_Alert:1448670089670037675> role delete",
    percentToHeat(ANTINUKE_CONFIG.roleDeletionFilter.heatPerAction || 25),
  );
  await enableAntiNukePanic(
    guild,
    "<a:VC_Alert:1448670089670037675> Role deletion filter",
  ...
  const minuteCount = state.minuteHits.length;
  const hourCount = state.hourHits.length;
  const exceededMinute = minuteCount >= Number(ANTINUKE_CONFIG.roleDeletionFilter.minuteLimit || 3);
  const exceededHour = hourCount >= Number(ANTINUKE_CONFIG.roleDeletionFilter.hourLimit || 10);
  const exceededHeat = state.heat >= 100;
  if (!exceededMinute && !exceededHour && !exceededHeat) return;
  ...
  const quarantine = await quarantineExecutor(guild, actorId, "<:VC_Lock:1482526739044368476> AntiNuke: role deletion spam detected",);
  await sendAntiNukeLog(
    guild,
    "<a:VC_Alert:1448670089670037675> AntiNuke: Role Deletions Filter",
    [
      formatExecutorLine(actorId),
      roleId
        ? `<a:VC_Alert:1448670089670037675> **Last Deleted Role:** ${roleName || "sconosciuto"} \`${roleId}\``
        : null,
      `<a:VC_Alert:1448670089670037675> **Minute Count:** ${minuteCount}/${ANTINUKE_CONFIG.roleDeletionFilter.minuteLimit}`,
      `<a:VC_Alert:1448670089670037675> **Hour Count:** ${hourCount}/${ANTINUKE_CONFIG.roleDeletionFilter.hourLimit}`,
      `<a:VC_Alert:1448670089670037675> **Heat:** ${state.heat}/100`,
      `<a:VC_Alert:1448670089670037675> **Result:** ${quarantineOutcomeLabel(quarantine)}`,
    ].filter(Boolean),
  );
...
async function handleChannelCreationAction({ guild, executorId, channelId = "", channel = null }) {
  ...
  await applyBurstPanicGuard(
    guild,
    actorId,
    "<a:VC_Alert:1448670089670037675> channel create",
    percentToHeat(ANTINUKE_CONFIG.channelCreationFilter.heatPerAction || 16),
  );
  await enableAntiNukePanic(
    guild,
    "<a:VC_Alert:1448670089670037675> Channel creation filter",
  ...
  if (panicActive && instantCfg?.enabled && instantCfg?.deleteCreatedChannels && channelId) {
    const me = guild.members?.me;
    const targetChannel = channel || (await getGuildChannelCached(guild, String(channelId), { ttlMs: 30_000 }));
    const canDelete = targetChannel?.deletable && me?.permissions?.has?.(PermissionsBitField.Flags.ManageChannels);
    if (canDelete) {
      const removed = await targetChannel.delete("<:VC_Lock:1482526739044368476> AntiNuke panic: delete channel created during active panic").then(() => true).catch(() => false);
      const canQuarantine = instantCfg?.quarantineExecutor && !shouldSkipPanicExecutorAction(guild.id, actorId, now);
      const quarantine = canQuarantine ? await quarantineExecutor(guild, actorId, "<:VC_Lock:1482526739044368476> AntiNuke panic: channel creation blocked during active panic",) : { applied: false, method: "cooldown" };
      if (removed) {
        await sendAntiNukeLog(
          guild,
          "<a:VC_Alert:1448670089670037675> AntiNuke: Panic Instant Rollback",
          [
            formatExecutorLine(actorId),
            `<a:VC_Alert:1448670089670037675> **Action:** Channel creation blocked during panic`,
            `<a:VC_Alert:1448670089670037675> **Channel:** \`${channelId}\``,
            `<a:VC_Alert:1448670089670037675> **Result:** Channel deleted immediately`,
            `<a:VC_Alert:1448670089670037675> **Quarantena executor:** ${quarantineOutcomeLabel(quarantine)}`,
          ],
        );
      }
  ...
  const minuteCount = state.minuteHits.length;
  const hourCount = state.hourHits.length;
  const exceededMinute = minuteCount >= Number(ANTINUKE_CONFIG.channelCreationFilter.minuteLimit || 4);
  const exceededHour = hourCount >= Number(ANTINUKE_CONFIG.channelCreationFilter.hourLimit || 12);
  const exceededHeat = state.heat >= 100;
  if (!exceededMinute && !exceededHour && !exceededHeat) return;
  ...
  const quarantine = await quarantineExecutor(guild, actorId, "<:VC_Lock:1482526739044368476> AntiNuke: channel creation spam detected",);
  await sendAntiNukeLog(
    guild,
    "<a:VC_Alert:1448670089670037675> AntiNuke: Channel Creations Filter",
    [
      formatExecutorLine(actorId),
      channelId
        ? `<a:VC_Alert:1448670089670037675> **Last Channel:** <#${channelId}> \`${channelId}\``
        : null,
      `<a:VC_Alert:1448670089670037675> **Minute Count:** ${minuteCount}/${ANTINUKE_CONFIG.channelCreationFilter.minuteLimit}`,
      `<a:VC_Alert:1448670089670037675> **Hour Count:** ${hourCount}/${ANTINUKE_CONFIG.channelCreationFilter.hourLimit}`,
      `<a:VC_Alert:1448670089670037675> **Heat:** ${state.heat}/100`,
      `<a:VC_Alert:1448670089670037675> **Result:** ${quarantineOutcomeLabel(quarantine)}`,
    ].filter(Boolean),
  );
...
async function handleChannelDeletionAction({ guild, executorId, channelName = "", channelId = "", channel = null }) {
  ...
  await applyBurstPanicGuard(
    guild,
    actorId,
    "<a:VC_Alert:1448670089670037675> channel delete",
    percentToHeat(ANTINUKE_CONFIG.channelDeletionFilter.heatPerAction || 25),
  );
  await enableAntiNukePanic(
    guild,
    "<a:VC_Alert:1448670089670037675> Channel deletion filter",
  ...
  const minuteCount = state.minuteHits.length;
  const hourCount = state.hourHits.length;
  const exceededMinute = minuteCount >= Number(ANTINUKE_CONFIG.channelDeletionFilter.minuteLimit || 3);
  const exceededHour = hourCount >= Number(ANTINUKE_CONFIG.channelDeletionFilter.hourLimit || 8);
  const exceededHeat = state.heat >= 100;
  if (!exceededMinute && !exceededHour && !exceededHeat) return;
  ...
  const quarantine = await quarantineExecutor(guild, actorId, "<:VC_Lock:1482526739044368476> AntiNuke: channel deletion spam detected",);
  await sendAntiNukeLog(
    guild,
    "<a:VC_Alert:1448670089670037675> AntiNuke: Channel Deletions Filter",
    [
      formatExecutorLine(actorId),
      channelId
        ? `<a:VC_Alert:1448670089670037675> **Last Deleted Channel:** ${channelName || "sconosciuto"} \`${channelId}\``
        : null,
      `<a:VC_Alert:1448670089670037675> **Minute Count:** ${minuteCount}/${ANTINUKE_CONFIG.channelDeletionFilter.minuteLimit}`,
      `<a:VC_Alert:1448670089670037675> **Hour Count:** ${hourCount}/${ANTINUKE_CONFIG.channelDeletionFilter.hourLimit}`,
      `<a:VC_Alert:1448670089670037675> **Heat:** ${state.heat}/100`,
      `<a:VC_Alert:1448670089670037675> **Result:** ${quarantineOutcomeLabel(quarantine)}`,
    ].filter(Boolean),
  );
...
async function handleWebhookCreationAction({ guild, executorId, webhookId = "", channelId = "" }) {
  ...
  await applyBurstPanicGuard(
    guild,
    actorId,
    "<a:VC_Alert:1448670089670037675> webhook create",
    percentToHeat(ANTINUKE_CONFIG.webhookCreationFilter.heatPerAction || 15),
  );
  await enableAntiNukePanic(
    guild,
    "<a:VC_Alert:1448670089670037675> Webhook creation filter",
  ...
  if (
    ...
    const canQuarantine = instantCfg?.quarantineExecutor && !shouldSkipPanicExecutorAction(guild.id, actorId, now);
    const quarantine = canQuarantine ? await quarantineExecutor(guild, actorId, "<:VC_Lock:1482526739044368476> AntiNuke panic: webhook creation blocked during active panic",) : { applied: false, method: "cooldown" };
    if (removed) {
      await sendAntiNukeLog(
        guild,
        "<a:VC_Alert:1448670089670037675> AntiNuke: Panic Instant Rollback",
        [
          formatExecutorLine(actorId),
          `<a:VC_Alert:1448670089670037675> **Action:** Webhook creation blocked during panic`,
          `<a:VC_Alert:1448670089670037675> **Webhook:** \`${normalizedWebhookId}\``,
          `<a:VC_Alert:1448670089670037675> **Result:** Webhook deleted immediately`,
          `<a:VC_Alert:1448670089670037675> **Quarantena executor:** ${quarantineOutcomeLabel(quarantine)}`,
        ],
      );
    }
  ...
  const minuteCount = state.minuteHits.length;
  const hourCount = state.hourHits.length;
  const exceededMinute = minuteCount >= Number(ANTINUKE_CONFIG.webhookCreationFilter.minuteLimit || 3);
  const exceededHour = hourCount >= Number(ANTINUKE_CONFIG.webhookCreationFilter.hourLimit || 10);
  const exceededHeat = state.heat >= 100;
  if (!exceededMinute && !exceededHour && !exceededHeat) return;
  ...
  const quarantine = await quarantineExecutor(guild, actorId, "<:VC_Lock:1482526739044368476> AntiNuke: webhook creation spam detected",);
  await sendAntiNukeLog(
    guild,
    "<a:VC_Alert:1448670089670037675> AntiNuke: Webhook Creations Filter",
    [
      formatExecutorLine(actorId),
      normalizedWebhookId
        ? `<a:VC_Alert:1448670089670037675> **Last Webhook:** \`${normalizedWebhookId}\``
        : null,
      `<a:VC_Alert:1448670089670037675> **Minute Count:** ${minuteCount}/${ANTINUKE_CONFIG.webhookCreationFilter.minuteLimit}`,
      `<a:VC_Alert:1448670089670037675> **Hour Count:** ${hourCount}/${ANTINUKE_CONFIG.webhookCreationFilter.hourLimit}`,
      `<a:VC_Alert:1448670089670037675> **Heat:** ${state.heat}/100`,
      `<a:VC_Alert:1448670089670037675> **Result:** ${quarantineOutcomeLabel(quarantine)}`,
    ].filter(Boolean),
  );
...
async function handleWebhookUpdateAction({ guild, executorId, webhookId = "" }) {
  ...
  const normalizedWebhookId = String(webhookId || "").trim();
  const now = Date.now();
  const trackerId = getTrackerExecutorId(actorId, normalizedWebhookId, "webhook:update", now,);
  if (
    shouldSkipDuplicatedActionEvent({
      guildId: guild.id,
      executorId: actorId,
  ...
  await applyBurstPanicGuard(
    guild,
    actorId,
    "<a:VC_Alert:1448670089670037675> webhook update",
    percentToHeat(ANTINUKE_CONFIG.webhookUpdateFilter.heatPerAction || 12),
  );
  await enableAntiNukePanic(
    guild,
    "<a:VC_Alert:1448670089670037675> Webhook update filter",
  ...
  const minuteCount = state.minuteHits.length;
  const hourCount = state.hourHits.length;
  const exceededMinute = minuteCount >= Number(ANTINUKE_CONFIG.webhookUpdateFilter.minuteLimit || 4);
  const exceededHour = hourCount >= Number(ANTINUKE_CONFIG.webhookUpdateFilter.hourLimit || 12);
  const exceededHeat = state.heat >= 100;
  if (!exceededMinute && !exceededHour && !exceededHeat) return;
  ...
  const quarantine = await quarantineExecutor(guild, actorId, "<:VC_Lock:1482526739044368476> AntiNuke: webhook update spam detected",);
  await sendAntiNukeLog(
    guild,
    "<a:VC_Alert:1448670089670037675> AntiNuke: Webhook Updates Filter",
    [
      formatExecutorLine(actorId),
      normalizedWebhookId
        ? `<a:VC_Alert:1448670089670037675> **Last Webhook:** \`${normalizedWebhookId}\``
        : null,
      `<a:VC_Alert:1448670089670037675> **Minute Count:** ${minuteCount}/${ANTINUKE_CONFIG.webhookUpdateFilter.minuteLimit}`,
      `<a:VC_Alert:1448670089670037675> **Hour Count:** ${hourCount}/${ANTINUKE_CONFIG.webhookUpdateFilter.hourLimit}`,
      `<a:VC_Alert:1448670089670037675> **Heat:** ${state.heat}/100`,
      `<a:VC_Alert:1448670089670037675> **Result:** ${quarantineOutcomeLabel(quarantine)}`,
    ].filter(Boolean),
  );
...
async function handleWebhookDeletionAction({ guild, executorId, webhookId = "" }) {
  ...
  await applyBurstPanicGuard(
    guild,
    actorId,
    "<a:VC_Alert:1448670089670037675> webhook delete",
    percentToHeat(ANTINUKE_CONFIG.webhookDeletionFilter.heatPerAction || 10),
  );
  await enableAntiNukePanic(
    guild,
    "<a:VC_Alert:1448670089670037675> Webhook deletion filter",
  ...
  const minuteCount = state.minuteHits.length;
  const hourCount = state.hourHits.length;
  const exceededMinute = minuteCount >= Number(ANTINUKE_CONFIG.webhookDeletionFilter.minuteLimit || 3);
  const exceededHour = hourCount >= Number(ANTINUKE_CONFIG.webhookDeletionFilter.hourLimit || 8);
  const exceededHeat = state.heat >= 100;
  if (!exceededMinute && !exceededHour && !exceededHeat) return;
  ...
  const quarantine = await quarantineExecutor(guild, actorId, "<:VC_Lock:1482526739044368476> AntiNuke: webhook deletion spam detected",);
  await sendAntiNukeLog(
    guild,
    "<a:VC_Alert:1448670089670037675> AntiNuke: Webhook Deletions Filter",
    [
      formatExecutorLine(actorId),
      normalizedWebhookId
        ? `<a:VC_Alert:1448670089670037675> **Last Webhook:** \`${normalizedWebhookId}\``
        : null,
      `<a:VC_Alert:1448670089670037675> **Minute Count:** ${minuteCount}/${ANTINUKE_CONFIG.webhookDeletionFilter.minuteLimit}`,
      `<a:VC_Alert:1448670089670037675> **Hour Count:** ${hourCount}/${ANTINUKE_CONFIG.webhookDeletionFilter.hourLimit}`,
      `<a:VC_Alert:1448670089670037675> **Heat:** ${state.heat}/100`,
      `<a:VC_Alert:1448670089670037675> **Result:** ${quarantineOutcomeLabel(quarantine)}`,
    ].filter(Boolean),
  );
...
async function handleInviteCreationAction({ guild, executorId, inviteCode = "", channelId = "" }) {
  ...
  await applyBurstPanicGuard(
    guild,
    actorId,
    "<a:VC_Alert:1448670089670037675> invite create",
    percentToHeat(ANTINUKE_CONFIG.inviteCreationFilter.heatPerAction || 12),
  );
  await enableAntiNukePanic(
    guild,
    "<a:VC_Alert:1448670089670037675> Invite creation filter",
  ...
  const minuteCount = state.minuteHits.length;
  const hourCount = state.hourHits.length;
  const exceededMinute = minuteCount >= Number(ANTINUKE_CONFIG.inviteCreationFilter.minuteLimit || 4);
  const exceededHour = hourCount >= Number(ANTINUKE_CONFIG.inviteCreationFilter.hourLimit || 15);
  const exceededHeat = state.heat >= 100;
  if (!exceededMinute && !exceededHour && !exceededHeat) return;
  ...
  const quarantine = await quarantineExecutor(guild, actorId, "<:VC_Lock:1482526739044368476> AntiNuke: invite creation spam detected",);
  await sendAntiNukeLog(
    guild,
    "<a:VC_Alert:1448670089670037675> AntiNuke: Invite Creations Filter",
    [
      formatExecutorLine(actorId),
      inviteCode
        ? `<a:VC_Alert:1448670089670037675> **Invite:** \`${inviteCode}\``
        : null,
      channelId
        ? `<a:VC_Alert:1448670089670037675> **Channel:** <#${channelId}> \`${channelId}\``
        : null,
      `<a:VC_Alert:1448670089670037675> **Minute Count:** ${minuteCount}/${ANTINUKE_CONFIG.inviteCreationFilter.minuteLimit}`,
      `<a:VC_Alert:1448670089670037675> **Hour Count:** ${hourCount}/${ANTINUKE_CONFIG.inviteCreationFilter.hourLimit}`,
      `<a:VC_Alert:1448670089670037675> **Heat:** ${state.heat}/100`,
      `<a:VC_Alert:1448670089670037675> **Result:** ${quarantineOutcomeLabel(quarantine)}`,
    ].filter(Boolean),
  );
...
async function handleRoleUpdate({ oldRole, newRole, executorId }) {
  ...
  const actorId = normalizeExecutorId(executorId);
  const addedDanger = dangerousAddedBits(oldRole?.permissions?.bitfield || 0n, newRole?.permissions?.bitfield || 0n, DANGEROUS_PERMS,);
  if (!addedDanger.length) return;
  if (isUnknownExecutorId(actorId)) {
    await logAuditMissingSkip(guild, "<a:VC_Alert:1448670089670037675> AntiNuke: Role Quarantine", [
      `<a:VC_Alert:1448670089670037675> **Role:** ${newRole} \`${newRole.id}\``,
      `<a:VC_Alert:1448670089670037675> **Action:** Dangerous permissions detected`,
    ]);
    return;
  }
  if (await isWhitelistedExecutorAsync(guild, actorId)) return;
  await enableAntiNukePanic(guild, "<a:VC_Alert:1448670089670037675> Dangerous role permission update", 100);
  ...
  await newRole.setPermissions(oldRole.permissions.bitfield, "<:VC_Lock:1482526739044368476> AntiNuke: revert dangerous role perms").catch(() => { });
  const quarantine = await quarantineExecutor(guild, actorId, "<:VC_Lock:1482526739044368476> AntiNuke: dangerous role permission update",);
  await sendAntiNukeLog(
    guild,
    "<a:VC_Alert:1448670089670037675> AntiNuke: Role Quarantine",
    [
      formatExecutorLine(actorId),
      `<a:VC_Alert:1448670089670037675> **Role:** ${newRole} \`${newRole.id}\``,
      `<a:VC_Alert:1448670089670037675> **Action:** Dangerous permissions reverted`,
      `<a:VC_Alert:1448670089670037675> **Result:** ${quarantineOutcomeLabel(quarantine)}`,
    ],
  );
}
...
async function handleMemberRoleAddition({ guild, targetMember, addedRoles, executorId }) {
  ...
  if (isUnknownExecutorId(actorId)) {
    global.logger?.warn?.(
      "<a:VC_Alert:1448670089670037675> [AntiNuke] Skipping dangerous member role rollback because audit executor is missing:",
      guild.id,
      targetMember.id,
    ...
    await sendAntiNukeLog(
      guild,
      "<a:VC_Alert:1448670089670037675> AntiNuke: Member Role Quarantine",
      [
        formatExecutorLine(actorId),
        formatAuditStatusLine(actorId),
        `<a:VC_Alert:1448670089670037675> **Target:** ${targetMember.user} \`${targetMember.id}\``,
        `<a:VC_Alert:1448670089670037675> **Action:** Dangerous granted role(s) detected`,
        `<:VC_reason:1478517122929004544> **Technical Reason:** Audit log entry not available in time, automatic rollback skipped to avoid false positives`,
        `<:VC_reason:1478517122929004544> **Result:** No rollback performed`,
      ],
      "#FEE75C",
    );
    return;
  }
  const dangerousRoles = addedRoles.filter((role) => containsDangerousBits(role?.permissions?.bitfield || 0n, DANGEROUS_PERMS),);
  if (!dangerousRoles.length) return;
  if (await isWhitelistedExecutorAsync(guild, actorId)) return;
  await enableAntiNukePanic(guild, "<a:VC_Alert:1448670089670037675> Dangerous role added to member", 100);
  ...
  await targetMember.roles.remove(removable, "<:VC_Lock:1482526739044368476> AntiNuke: remove dangerous role grants").catch(() => { });
  ...
  const quarantine = await quarantineExecutor(guild, actorId, "<:VC_Lock:1482526739044368476> AntiNuke: dangerous role granted to member",);
  await sendAntiNukeLog(
    guild,
    "<a:VC_Alert:1448670089670037675> AntiNuke: Member Role Quarantine",
    [
      ...
      formatAuditStatusLine(actorId),
      `<a:VC_Alert:1448670089670037675> **Target:** ${targetMember.user} \`${targetMember.id}\``,
      `<a:VC_Alert:1448670089670037675> **Action:** Dangerous granted role(s) removed`,
      `<a:VC_Alert:1448670089670037675> **Result:** ${quarantineOutcomeLabel(quarantine)}`,
    ],
  );
}

async function handleChannelOverwrite({ guild, channel, overwrite, beforeAllow = 0n, afterAllow = 0n, executorId }) {
  ...
  if (!addedDanger.length) return;
  if (isUnknownExecutorId(actorId)) {
    await logAuditMissingSkip(guild, "<a:VC_Alert:1448670089670037675> AntiNuke: Channel Overwrite Quarantine", [
      `<a:VC_Alert:1448670089670037675> **Channel:** ${channel} \`${channel.id}\``,
      `<a:VC_Alert:1448670089670037675> **Overwrite Role:** <@&${overwrite.id}> \`${overwrite.id}\``,
      `<a:VC_Alert:1448670089670037675> **Action:** Dangerous channel perms detected`,
    ]);
    return;
  }
  if (await isWhitelistedExecutorAsync(guild, actorId)) return;
  await enableAntiNukePanic(guild, "<a:VC_Alert:1448670089670037675> Dangerous channel overwrite", 100);
  ...
  await channel.permissionOverwrites.edit(
    ...
    { reason: "<:VC_Lock:1482526739044368476> AntiNuke: revert dangerous channel overwrite permissions" },
  ).catch(() => { });

  const quarantine = await quarantineExecutor(guild, actorId, "<:VC_Lock:1482526739044368476> AntiNuke: dangerous channel overwrite",);
  await sendAntiNukeLog(
    guild,
    "<a:VC_Alert:1448670089670037675> AntiNuke: Channel Overwrite Quarantine",
    [
      formatExecutorLine(actorId),
      `<a:VC_Alert:1448670089670037675> **Channel:** ${channel} \`${channel.id}\``,
      `<a:VC_Alert:1448670089670037675> **Overwrite Role:** <@&${overwrite.id}> \`${overwrite.id}\``,
      `<a:VC_Alert:1448670089670037675> **Action:** Dangerous channel perms reverted`,
      `<a:VC_Alert:1448670089670037675> **Result:** ${quarantineOutcomeLabel(quarantine)}`,
    ],
  );
}
...
async function handleVanityGuard({ oldGuild, newGuild, executorId }) {
  ...
  if (isUnknownExecutorId(actorId)) {
    await logAuditMissingSkip(guild, "<a:VC_Alert:1448670089670037675> AntiNuke: Vanity Guard", [
      oldVanity
        ? `<a:VC_Alert:1448670089670037675> **Action:** Unauthorized vanity change detected for \`${oldVanity}\``
        : `<a:VC_Alert:1448670089670037675> **Action:** Unauthorized vanity set detected`,
    ]);
    return;
  }
  if (!newGuild?.setVanityCode) return;
  if (await isWhitelistedExecutorAsync(guild, actorId)) return;
  await enableAntiNukePanic(guild, "<a:VC_Alert:1448670089670037675> Vanity URL unauthorized update", 100);

  if (oldVanity) {
    await newGuild.setVanityCode(oldVanity, "<:VC_Lock:1482526739044368476> AntiNuke: restore vanity url").catch(() => { });
  } else {
    await newGuild.setVanityCode(null, "<:VC_Lock:1482526739044368476> AntiNuke: clear unauthorized vanity url").catch(() => { });
  }
  const quarantine = await quarantineExecutor(guild, actorId, "<:VC_Lock:1482526739044368476> AntiNuke: unauthorized vanity update",);
  await sendAntiNukeLog(
    guild,
    "<a:VC_Alert:1448670089670037675> AntiNuke: Vanity Guard",
    [
      formatExecutorLine(actorId),
      oldVanity
        ? `<a:VC_Alert:1448670089670037675> **Action:** Vanity restored to \`${oldVanity}\``
        : `<a:VC_Alert:1448670089670037675> **Action:** Unauthorized vanity removed`,
      `<a:VC_Alert:1448670089670037675> **Result:** ${quarantineOutcomeLabel(quarantine)}`,
    ],
  );
}
...
async function handlePruneAction({ guild, executorId, removedCount = 0 }) {
  ...
  const actorId = normalizeExecutorId(executorId);
  if (isUnknownExecutorId(actorId)) {
    await logAuditMissingSkip(guild, "<a:VC_Alert:1448670089670037675> AntiNuke: Member Prune", [
      `<a:VC_Alert:1448670089670037675> **Action:** Member prune detected`,
      `<a:VC_Alert:1448670089670037675> **Rimossi:** ${Number(removedCount || 0)}`,
    ]);
    return;
  }
  if (await isWhitelistedExecutorAsync(guild, actorId)) return;
  await enableAntiNukePanic(guild, "<a:VC_Alert:1448670089670037675> Member prune detected", 100);
  const quarantine = await quarantineExecutor(guild, actorId, "<:VC_Lock:1482526739044368476> AntiNuke: member prune detected",);
  await sendAntiNukeLog(
    guild,
    "<a:VC_Alert:1448670089670037675> AntiNuke: Member Prune",
    [
      formatExecutorLine(actorId),
      `<a:VC_Alert:1448670089670037675> **Action:** Member prune`,
      `<a:VC_Alert:1448670089670037675> **Rimossi:** ${Number(removedCount || 0)}`,
      `<a:VC_Alert:1448670089670037675> **Result:** ${quarantineOutcomeLabel(quarantine)}`,
    ],
  );
...
async function handleThreadCreationAction({ guild, executorId, threadId = "", thread = null }) {
...
async function handleThreadDeletionAction({ guild, executorId, threadName = "", threadId = "", thread = null }) {
...
function getAntiNukeStatusSnapshot(guildId = "") {
  ...
  const trackerSizes = { kickBan: KICK_BAN_TRACKER.size, roleCreate: ROLE_CREATION_TRACKER.size, roleDelete: ROLE_DELETION_TRACKER.size, channelCreate: CHANNEL_CREATION_TRACKER.size, channelDelete: CHANNEL_DELETION_TRACKER.size, webhookCreate: WEBHOOK_CREATION_TRACKER.size, webhookUpdate: WEBHOOK_UPDATE_TRACKER.size, webhookDelete: WEBHOOK_DELETION_TRACKER.size, inviteCreate: INVITE_CREATION_TRACKER.size, };
...
async function stopAntiNukePanic(guild, reason = "manual stop", stoppedById = "") {
  ...
  const restored = Boolean(roleResult?.completed) && Boolean(channelResult?.completed) && Number(state.lockedRoles?.size || 0) === 0 && Number(state.lockedChannels?.size || 0) === 0;
  if (!restored) {
...
async function shouldBlockModerationCommands(guild, userId) {
  ...
  const member = guild.members.cache.get(String(userId)) || (await getGuildMemberCached(guild, String(userId), { ttlMs: 30_000 }));
  const emergencyBypassRoleIds = new Set([IDs.roles.Founder, IDs.roles.CoFounder,].filter(Boolean).map(String),);
  if (
    member &&
...
module.exports = { ANTINUKE_CONFIG, ANTINUKE_PRESETS, handleRoleUpdate, handleMemberRoleAddition, handleChannelOverwrite, handleVanityGuard, handleKickBanAction, handleRoleCreationAction, handleRoleDeletionAction, handleChannelCreationAction, handleChannelDeletionAction, handleWebhookCreationAction, handleWebhookUpdateAction, handleWebhookDeletionAction, handleInviteCreationAction, handleThreadCreationAction, handleThreadDeletionAction, handlePruneAction, isAntiNukePanicActive, shouldBlockModerationCommands, shouldBlockAllCommands, isWhitelistedExecutor, isWhitelistedExecutorAsync, applyAntiNukePreset, getAntiNukeStatusSnapshot, stopAntiNukePanic, triggerAntiNukePanicExternal, setAntiNukeConfigSnapshot, addMaintenanceAllowlistUser, removeMaintenanceAllowlistUser, listMaintenanceAllowlist };