...
async function buildServerOverviewPayload(guild, lookbackDays, authorId) {
  ...
  const customId = authorId
    ? `${SERVER_REFRESH_CUSTOM_ID_PREFIX}:${authorId}:${safeLookback}:embed`
    : `${SERVER_REFRESH_CUSTOM_ID_PREFIX}:${safeLookback}:embed`;
