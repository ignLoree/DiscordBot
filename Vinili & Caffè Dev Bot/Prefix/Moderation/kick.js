...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    if (member.roles.highest.position >= message.member.roles.highest.position) {
      return message.reply({ content: '<:vegax:1443934876440068179> Non puoi kickare un utente con ruolo uguale o superiore.' });
    }

    if (!member.kickable) return message.reply({ content: '<:vegax:1443934876440068179> Non posso kickare questo utente.' });
