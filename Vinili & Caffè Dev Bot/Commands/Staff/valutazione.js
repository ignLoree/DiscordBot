...
module.exports = {
    ...
    async execute(interaction) {
        ...
        try {
            ...
            if (group === "positive") {
                if (!checkPermissions()) return await interaction.editReply({ embeds: [new EmbedBuilder().setDescription("<:vegax:1443934876440068179> Non hai il permesso per fare questo comando!").setColor("Red")], flags: 1 << 6 });
                if (sub === "add") {
                    ...
                    return await interaction.editReply({ embeds: [new EmbedBuilder().setDescription("<:vegacheckmark:1443666279058772028> Valutazione positiva registrata con successo!").setColor('#6f4e37')] });
                }
                if (sub === "remove") {
                    const removeId = interaction.options.getInteger("id");
                    if (!StaffDoc.positiveReasons[removeId - 1]) return await interaction.editReply({ embeds: [new EmbedBuilder().setDescription("<:vegax:1443934876440068179> ID non valido").setColor("Red")], flags: 1 << 6 });
                    ...
                    return await interaction.editReply({ embeds: [new EmbedBuilder().setDescription("<:vegacheckmark:1443666279058772028> Valutazione positiva rimossa con successo!").setColor('#6f4e37')] });
                }
            }
            if (group === "negative") {
                if (!checkPermissions()) return await interaction.editReply({ embeds: [new EmbedBuilder().setDescription("<:vegax:1443934876440068179> Non hai il permesso per fare questo comando!").setColor("Red")], flags: 1 << 6 });
                if (sub === "add") {
                    ...
                    const embed = new EmbedBuilder()
                        ...
                        .setFooter({ text: `© 2025 Vinili & Caffè. Tutti i diritti riservati.`, iconURL: `${interaction.guild.iconURL()}` });
                    await channel.send({ content: `${utentee}`, embeds: [embed] });
                    return await interaction.editReply({ embeds: [new EmbedBuilder().setDescription("<:vegacheckmark:1443666279058772028> Valutazione negativa registrata con successo!").setColor('#6f4e37')] });
                }
                if (sub === "remove") {
                    const removeId = interaction.options.getInteger("id");
                    if (!StaffDoc.negativeReasons[removeId - 1]) return await interaction.editReply({ embeds: [new EmbedBuilder().setDescription("<:vegax:1443934876440068179> ID non valido").setColor("Red")], flags: 1 << 6 });
                    ...
                    const embed = new EmbedBuilder()
                        ...
                        .setFooter({ text: `© 2025 Vinili & Caffè. Tutti i diritti riservati.`, iconURL: `${interaction.guild.iconURL()}` });
                    await channel.send({ embeds: [embed] });
                    return await interaction.editReply({ embeds: [new EmbedBuilder().setDescription("<:vegacheckmark:1443666279058772028> Valutazione negativa rimossa con successo!").setColor('#6f4e37')] });
                }
            }
            if (sub === "media") {
                const doc = await Staff.findOne({ guildId: interaction.guild.id, userId: utentee.id });
                if (!doc) return await interaction.editReply({ embeds: [new EmbedBuilder().setDescription("<:attentionfromvega:1443651874032062505> Nessuna valutazione trovata.").setColor("Red")], flags: 1 << 6 });
                const embed = new EmbedBuilder()
